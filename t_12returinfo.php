...
//
class ct_12retur extends cTable {
	...
	function __construct() {
		...
		// retur_id
		$this->retur_id = new cField('t_12retur', 't_12retur', 'x_retur_id', 'retur_id', '`retur_id`', '`retur_id`', 3, -1, FALSE, '`retur_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->retur_id->Sortable = TRUE; // Allow sort
		$this->retur_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['retur_id'] = &$this->retur_id;

		// tgl
		$this->tgl = new cField('t_12retur', 't_12retur', 'x_tgl', 'tgl', '`tgl`', ew_CastDateFieldForLike('`tgl`', 7, "DB"), 133, 7, FALSE, '`tgl`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// item_id
		$this->item_id = new cField('t_12retur', 't_12retur', 'x_item_id', 'item_id', '`item_id`', '`item_id`', 3, -1, FALSE, '`EV__item_id`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'TEXT');
		...
		// qty
		$this->qty = new cField('t_12retur', 't_12retur', 'x_qty', 'qty', '`qty`', '`qty`', 4, -1, FALSE, '`qty`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->qty->Sortable = TRUE; // Allow sort
		$this->qty->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['qty'] = &$this->qty;

		// satuan_id
		$this->satuan_id = new cField('t_12retur', 't_12retur', 'x_satuan_id', 'satuan_id', '`satuan_id`', '`satuan_id`', 3, -1, FALSE, '`EV__satuan_id`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'TEXT');
		...
		// jual_id
		$this->jual_id = new cField('t_12retur', 't_12retur', 'x_jual_id', 'jual_id', '`jual_id`', '`jual_id`', 3, -1, FALSE, '`EV__jual_id`', TRUE, TRUE, TRUE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlSelectList() { // Select for List page
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT `item_nama` FROM `t_02item` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`item_id` = `t_12retur`.`item_id` LIMIT 1) AS `EV__item_id`, (SELECT `satuan_nama` FROM `t_03satuan` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`satuan_id` = `t_12retur`.`satuan_id` LIMIT 1) AS `EV__satuan_id`, (SELECT `no_po` FROM `t_06jual` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`jual_id` = `t_12retur`.`jual_id` LIMIT 1) AS `EV__jual_id` FROM `t_12retur`" .
			") `EW_TMP_TABLE`";
	...
	function UseVirtualFields() {
		...
		if (strpos($sOrderBy, " " . $this->satuan_id->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		if ($this->jual_id->AdvancedSearch->SearchValue <> "" ||
			$this->jual_id->AdvancedSearch->SearchValue2 <> "" ||
			strpos($sWhere, " " . $this->jual_id->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		if (strpos($sOrderBy, " " . $this->jual_id->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		return FALSE;
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->retur_id->setDbValue($conn->Insert_ID());
			$rs['retur_id'] = $this->retur_id->DbValue;
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			if (array_key_exists('retur_id', $rs))
				ew_AddFilter($where, ew_QuotedName('retur_id', $this->DBID) . '=' . ew_QuotedValue($rs['retur_id'], $this->retur_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`retur_id` = @retur_id@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->retur_id->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@retur_id@", ew_AdjustSql($this->retur_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t_12returview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t_12returview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "t_12returadd.php?" . $this->UrlParm($parm);
		else
			$url = "t_12returadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("t_12returedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("t_12returadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("t_12returdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "retur_id:" . ew_VarToJson($this->retur_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->retur_id->CurrentValue)) {
			$sUrl .= "retur_id=" . urlencode($this->retur_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["retur_id"]))
				$arKeys[] = ew_StripSlashes($_POST["retur_id"]);
			elseif (isset($_GET["retur_id"]))
				$arKeys[] = ew_StripSlashes($_GET["retur_id"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->retur_id->CurrentValue = $key;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->retur_id->Exportable) $Doc->ExportCaption($this->retur_id);
					if ($this->tgl->Exportable) $Doc->ExportCaption($this->tgl);
					if ($this->item_id->Exportable) $Doc->ExportCaption($this->item_id);
					if ($this->qty->Exportable) $Doc->ExportCaption($this->qty);
					if ($this->satuan_id->Exportable) $Doc->ExportCaption($this->satuan_id);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->retur_id->Exportable) $Doc->ExportField($this->retur_id);
						if ($this->tgl->Exportable) $Doc->ExportField($this->tgl);
						if ($this->item_id->Exportable) $Doc->ExportField($this->item_id);
						if ($this->qty->Exportable) $Doc->ExportField($this->qty);
						if ($this->satuan_id->Exportable) $Doc->ExportField($this->satuan_id);
