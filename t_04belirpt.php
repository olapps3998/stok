...
<?php include_once "t_04belirptinfo.php" ?>
<?php
...
$t_04beli_rpt = NULL; // Initialize page object first

class crt_04beli_rpt extends crt_04beli {
	...
	function __construct() {
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 't_04beli', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption ft_04belirpt";
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_t_04beli\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_t_04beli',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"ft_04belirpt\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"ft_04belirpt\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"ft_04belirpt\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = FALSE;
	...
	function Page_Main() {
		...
		$this->Col = array(array(FALSE, FALSE), array(FALSE,FALSE), array(FALSE,FALSE), array(FALSE,FALSE), array(FALSE,FALSE), array(FALSE,FALSE), array(FALSE,FALSE), array(FALSE,FALSE), array(TRUE,FALSE), array(FALSE,FALSE), array(FALSE,FALSE), array(FALSE,FALSE), array(TRUE,FALSE));
	...
	function GetRow($opt) {
		...
		if ($opt == 1) { // Get first row
				...
				$this->FirstRowData['vendor_id'] = ewr_Conv($rs->fields('vendor_id'), 3);
				$this->FirstRowData['item_id'] = ewr_Conv($rs->fields('item_id'), 3);
				$this->FirstRowData['qty'] = ewr_Conv($rs->fields('qty'), 3);
				$this->FirstRowData['satuan_id'] = ewr_Conv($rs->fields('satuan_id'), 3);
				$this->FirstRowData['harga'] = ewr_Conv($rs->fields('harga'), 3);
				$this->FirstRowData['sub_total'] = ewr_Conv($rs->fields('sub_total'), 20);
				$this->FirstRowData['tgl_dp'] = ewr_Conv($rs->fields('tgl_dp'), 133);
				$this->FirstRowData['jml_dp'] = ewr_Conv($rs->fields('jml_dp'), 5);
				$this->FirstRowData['tgl_lunas'] = ewr_Conv($rs->fields('tgl_lunas'), 133);
				$this->FirstRowData['jml_lunas'] = ewr_Conv($rs->fields('jml_lunas'), 5);
		} else { // Get next row
			$rs->MoveNext();
		}
		if (!$rs->EOF) {
			...
			$this->Val[3] = $this->vendor_id->CurrentValue;
			$this->Val[4] = $this->item_id->CurrentValue;
			$this->Val[5] = $this->qty->CurrentValue;
			$this->Val[6] = $this->satuan_id->CurrentValue;
			...
			$this->vendor_id->setDbValue("");
			$this->item_id->setDbValue("");
			$this->qty->setDbValue("");
			$this->satuan_id->setDbValue("");
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			$this->vendor_id->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
			...
			$this->satuan_id->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// vendor_id
			$CurrentValue = $this->vendor_id->CurrentValue;
			$ViewValue = &$this->vendor_id->ViewValue;
			$ViewAttrs = &$this->vendor_id->ViewAttrs;
			$CellAttrs = &$this->vendor_id->CellAttrs;
			$HrefValue = &$this->vendor_id->HrefValue;
			$LinkAttrs = &$this->vendor_id->LinkAttrs;
			$this->Cell_Rendered($this->vendor_id, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
			...
			// satuan_id
			$CurrentValue = $this->satuan_id->CurrentValue;
			$ViewValue = &$this->satuan_id->ViewValue;
			$ViewAttrs = &$this->satuan_id->ViewAttrs;
			$CellAttrs = &$this->satuan_id->CellAttrs;
			$HrefValue = &$this->satuan_id->HrefValue;
			$LinkAttrs = &$this->satuan_id->LinkAttrs;
			$this->Cell_Rendered($this->satuan_id, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
	...
	function SetupFieldCount() {
		...
		if ($this->vendor_id->Visible) $this->DtlColumnCount += 1;
		if ($this->item_id->Visible) $this->DtlColumnCount += 1;
		if ($this->qty->Visible) $this->DtlColumnCount += 1;
		if ($this->satuan_id->Visible) $this->DtlColumnCount += 1;
...
// Create page object
if (!isset($t_04beli_rpt)) $t_04beli_rpt = new crt_04beli_rpt();
...
// Create page object
var t_04beli_rpt = new ewr_Page("t_04beli_rpt");

// Page properties
t_04beli_rpt.PageID = "rpt"; // Page ID
var EWR_PAGE_ID = t_04beli_rpt.PageID;

// Extend page with Chart_Rendering function
t_04beli_rpt.Chart_Rendering = 
...
<?php include "t_04belirptpager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<!-- Report grid (begin) -->
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="tgl_beli"><div class="t_04beli_tgl_beli"><span class="ewTableHeaderCaption"><?php echo $Page->tgl_beli->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="tgl_beli">
<?php if ($Page->SortUrl($Page->tgl_beli) == "") { ?>
		<div class="ewTableHeaderBtn t_04beli_tgl_beli">
			<span class="ewTableHeaderCaption"><?php echo $Page->tgl_beli->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer t_04beli_tgl_beli" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->tgl_beli) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->tgl_beli->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="tgl_kirim"><div class="t_04beli_tgl_kirim"><span class="ewTableHeaderCaption"><?php echo $Page->tgl_kirim->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="tgl_kirim">
<?php if ($Page->SortUrl($Page->tgl_kirim) == "") { ?>
		<div class="ewTableHeaderBtn t_04beli_tgl_kirim">
			<span class="ewTableHeaderCaption"><?php echo $Page->tgl_kirim->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer t_04beli_tgl_kirim" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->tgl_kirim) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->tgl_kirim->FldCaption() ?></span>
...
<?php } ?>
<?php } ?>
<?php if ($Page->vendor_id->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="vendor_id"><div class="t_04beli_vendor_id"><span class="ewTableHeaderCaption"><?php echo $Page->vendor_id->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="vendor_id">
<?php if ($Page->SortUrl($Page->vendor_id) == "") { ?>
		<div class="ewTableHeaderBtn t_04beli_vendor_id">
			<span class="ewTableHeaderCaption"><?php echo $Page->vendor_id->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer t_04beli_vendor_id" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->vendor_id) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->vendor_id->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->vendor_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->vendor_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="item_id"><div class="t_04beli_item_id"><span class="ewTableHeaderCaption"><?php echo $Page->item_id->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="item_id">
<?php if ($Page->SortUrl($Page->item_id) == "") { ?>
		<div class="ewTableHeaderBtn t_04beli_item_id">
			<span class="ewTableHeaderCaption"><?php echo $Page->item_id->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer t_04beli_item_id" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->item_id) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->item_id->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="qty"><div class="t_04beli_qty" style="text-align: right;"><span class="ewTableHeaderCaption"><?php echo $Page->qty->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="qty">
<?php if ($Page->SortUrl($Page->qty) == "") { ?>
		<div class="ewTableHeaderBtn t_04beli_qty" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->qty->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer t_04beli_qty" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->qty) ?>',2);" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->qty->FldCaption() ?></span>
...
<?php } ?>
<?php } ?>
<?php if ($Page->satuan_id->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="satuan_id"><div class="t_04beli_satuan_id"><span class="ewTableHeaderCaption"><?php echo $Page->satuan_id->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="satuan_id">
<?php if ($Page->SortUrl($Page->satuan_id) == "") { ?>
		<div class="ewTableHeaderBtn t_04beli_satuan_id">
			<span class="ewTableHeaderCaption"><?php echo $Page->satuan_id->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer t_04beli_satuan_id" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->satuan_id) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->satuan_id->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->satuan_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->satuan_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="harga"><div class="t_04beli_harga" style="text-align: right;"><span class="ewTableHeaderCaption"><?php echo $Page->harga->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="harga">
<?php if ($Page->SortUrl($Page->harga) == "") { ?>
		<div class="ewTableHeaderBtn t_04beli_harga" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->harga->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer t_04beli_harga" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->harga) ?>',2);" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->harga->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="sub_total"><div class="t_04beli_sub_total" style="text-align: right;"><span class="ewTableHeaderCaption"><?php echo $Page->sub_total->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="sub_total">
<?php if ($Page->SortUrl($Page->sub_total) == "") { ?>
		<div class="ewTableHeaderBtn t_04beli_sub_total" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->sub_total->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer t_04beli_sub_total" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->sub_total) ?>',2);" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->sub_total->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="tgl_dp"><div class="t_04beli_tgl_dp"><span class="ewTableHeaderCaption"><?php echo $Page->tgl_dp->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="tgl_dp">
<?php if ($Page->SortUrl($Page->tgl_dp) == "") { ?>
		<div class="ewTableHeaderBtn t_04beli_tgl_dp">
			<span class="ewTableHeaderCaption"><?php echo $Page->tgl_dp->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer t_04beli_tgl_dp" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->tgl_dp) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->tgl_dp->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="jml_dp"><div class="t_04beli_jml_dp" style="text-align: right;"><span class="ewTableHeaderCaption"><?php echo $Page->jml_dp->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="jml_dp">
<?php if ($Page->SortUrl($Page->jml_dp) == "") { ?>
		<div class="ewTableHeaderBtn t_04beli_jml_dp" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->jml_dp->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer t_04beli_jml_dp" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->jml_dp) ?>',2);" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->jml_dp->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="tgl_lunas"><div class="t_04beli_tgl_lunas"><span class="ewTableHeaderCaption"><?php echo $Page->tgl_lunas->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="tgl_lunas">
<?php if ($Page->SortUrl($Page->tgl_lunas) == "") { ?>
		<div class="ewTableHeaderBtn t_04beli_tgl_lunas">
			<span class="ewTableHeaderCaption"><?php echo $Page->tgl_lunas->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer t_04beli_tgl_lunas" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->tgl_lunas) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->tgl_lunas->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="jml_lunas"><div class="t_04beli_jml_lunas" style="text-align: right;"><span class="ewTableHeaderCaption"><?php echo $Page->jml_lunas->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="jml_lunas">
<?php if ($Page->SortUrl($Page->jml_lunas) == "") { ?>
		<div class="ewTableHeaderBtn t_04beli_jml_lunas" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->jml_lunas->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer t_04beli_jml_lunas" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->jml_lunas) ?>',2);" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->jml_lunas->FldCaption() ?></span>
...
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_t_04beli_tgl_beli"<?php echo $Page->tgl_beli->ViewAttributes() ?>><?php echo $Page->tgl_beli->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->tgl_kirim->Visible) { ?>
		<td data-field="tgl_kirim"<?php echo $Page->tgl_kirim->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_t_04beli_tgl_kirim"<?php echo $Page->tgl_kirim->ViewAttributes() ?>><?php echo $Page->tgl_kirim->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->vendor_id->Visible) { ?>
		<td data-field="vendor_id"<?php echo $Page->vendor_id->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_t_04beli_vendor_id"<?php echo $Page->vendor_id->ViewAttributes() ?>><?php echo $Page->vendor_id->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->item_id->Visible) { ?>
		<td data-field="item_id"<?php echo $Page->item_id->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_t_04beli_item_id"<?php echo $Page->item_id->ViewAttributes() ?>><?php echo $Page->item_id->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->qty->Visible) { ?>
		<td data-field="qty"<?php echo $Page->qty->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_t_04beli_qty"<?php echo $Page->qty->ViewAttributes() ?>><?php echo $Page->qty->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->satuan_id->Visible) { ?>
		<td data-field="satuan_id"<?php echo $Page->satuan_id->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_t_04beli_satuan_id"<?php echo $Page->satuan_id->ViewAttributes() ?>><?php echo $Page->satuan_id->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->harga->Visible) { ?>
		<td data-field="harga"<?php echo $Page->harga->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_t_04beli_harga"<?php echo $Page->harga->ViewAttributes() ?>><?php echo $Page->harga->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->sub_total->Visible) { ?>
		<td data-field="sub_total"<?php echo $Page->sub_total->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_t_04beli_sub_total"<?php echo $Page->sub_total->ViewAttributes() ?>><?php echo $Page->sub_total->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->tgl_dp->Visible) { ?>
		<td data-field="tgl_dp"<?php echo $Page->tgl_dp->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_t_04beli_tgl_dp"<?php echo $Page->tgl_dp->ViewAttributes() ?>><?php echo $Page->tgl_dp->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->jml_dp->Visible) { ?>
		<td data-field="jml_dp"<?php echo $Page->jml_dp->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_t_04beli_jml_dp"<?php echo $Page->jml_dp->ViewAttributes() ?>><?php echo $Page->jml_dp->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->tgl_lunas->Visible) { ?>
		<td data-field="tgl_lunas"<?php echo $Page->tgl_lunas->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_t_04beli_tgl_lunas"<?php echo $Page->tgl_lunas->ViewAttributes() ?>><?php echo $Page->tgl_lunas->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->jml_lunas->Visible) { ?>
		<td data-field="jml_lunas"<?php echo $Page->jml_lunas->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_t_04beli_jml_lunas"<?php echo $Page->jml_lunas->ViewAttributes() ?>><?php echo $Page->jml_lunas->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
