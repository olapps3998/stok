...
//
// Table class for v_04stok
//
class cv_04stok extends cTable {
	var $item_id;
	var $item_nama;
	var $tgl;
	var $masuk;
	...
	function __construct() {
		...
		$this->TableVar = 'v_04stok';
		$this->TableName = 'v_04stok';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`v_04stok`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// item_id
		$this->item_id = new cField('v_04stok', 'v_04stok', 'x_item_id', 'item_id', '`item_id`', '`item_id`', 3, -1, FALSE, '`item_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// item_nama
		$this->item_nama = new cField('v_04stok', 'v_04stok', 'x_item_nama', 'item_nama', '`item_nama`', '`item_nama`', 200, -1, FALSE, '`item_nama`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->item_nama->Sortable = TRUE; // Allow sort
		$this->fields['item_nama'] = &$this->item_nama;

		// tgl
		$this->tgl = new cField('v_04stok', 'v_04stok', 'x_tgl', 'tgl', '`tgl`', ew_CastDateFieldForLike('`tgl`', 0, "DB"), 133, 0, FALSE, '`tgl`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// masuk
		$this->masuk = new cField('v_04stok', 'v_04stok', 'x_masuk', 'masuk', '`masuk`', '`masuk`', 4, -1, FALSE, '`masuk`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// keluar
		$this->keluar = new cField('v_04stok', 'v_04stok', 'x_keluar', 'keluar', '`keluar`', '`keluar`', 4, -1, FALSE, '`keluar`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// saldo
		$this->saldo = new cField('v_04stok', 'v_04stok', 'x_saldo', 'saldo', '`saldo`', '`saldo`', 4, -1, FALSE, '`saldo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("v_04stokview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("v_04stokview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "v_04stokadd.php?" . $this->UrlParm($parm);
		else
			$url = "v_04stokadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("v_04stokedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("v_04stokadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("v_04stokdelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) {
			...
			if (!$Doc->ExportCustom) {
				...
				if ($this->item_nama->Exportable) $Doc->ExportAggregate($this->item_nama, '');
				if ($this->tgl->Exportable) $Doc->ExportAggregate($this->tgl, '');
				if ($this->masuk->Exportable) $Doc->ExportAggregate($this->masuk, 'TOTAL');
				if ($this->keluar->Exportable) $Doc->ExportAggregate($this->keluar, 'TOTAL');
				if ($this->saldo->Exportable) $Doc->ExportAggregate($this->saldo, '');
				$Doc->EndExportRow();
			}
		}
		if (!$Doc->ExportCustom) {
	...
	function Row_Rendered() {
		...
		if ($GLOBALS["item_id"] == $this->item_id->CurrentValue) {
		}
		else {
		...
		$GLOBALS["item_id"] = $this->item_id->CurrentValue;
	}
