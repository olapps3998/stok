...
<?php include_once "r_transaksismryinfo.php" ?>
<?php
...
$r_transaksi_summary = NULL; // Initialize page object first

class crr_transaksi_summary extends crr_transaksi {
	...
	// Page object name
	var $PageObjName = 'r_transaksi_summary';
	...
	function __construct() {
		...
		// Table object (r_transaksi)
		if (!isset($GLOBALS["r_transaksi"])) {
			$GLOBALS["r_transaksi"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["r_transaksi"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'r_transaksi', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fr_transaksisummary";
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_r_transaksi\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_r_transaksi',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fr_transaksisummary\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fr_transaksisummary\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fr_transaksisummary\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = FALSE;
	...
	function Page_Main() {
		...
		$this->vendor_customer->SetVisibility();
		...
		$this->Col = array(array(FALSE, FALSE), array(FALSE,FALSE), array(FALSE,FALSE), array(FALSE,FALSE), array(TRUE,FALSE), array(FALSE,FALSE), array(FALSE,FALSE), array(TRUE,FALSE));
	...
	function GetRow($opt) {
		...
		if (!$rs->EOF) {
			...
			$this->Val[3] = $this->vendor_customer->CurrentValue;
			$this->Val[4] = $this->qty->CurrentValue;
			$this->Val[5] = $this->satuan_nama->CurrentValue;
			$this->Val[6] = $this->harga->CurrentValue;
			$this->Val[7] = $this->sub_total->CurrentValue;
		} else {
			$this->item_id->setDbValue("");
	...
	function RenderRow() {
		...
		if (!$this->GrandSummarySetup) { // Get Grand total
			...
			if ($rsagg) {
				...
				$this->GrandSmry[7] = $rsagg->fields("sum_sub_total");
				$rsagg->Close();
				$bGotSummary = TRUE;
			}
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			// qty
			$this->qty->SumViewValue = $this->qty->SumValue;
			$this->qty->SumViewValue = ewr_FormatNumber($this->qty->SumViewValue, 0, -2, -2, -2);
			$this->qty->CellAttrs["style"] = "text-align:right;";
			$this->qty->CellAttrs["class"] = ($this->RowTotalType == EWR_ROWTOTAL_PAGE || $this->RowTotalType == EWR_ROWTOTAL_GRAND) ? "ewRptGrpAggregate" : "ewRptGrpSummary" . $this->RowGroupLevel;
			...
			// vendor_customer
			$this->vendor_customer->HrefValue = "";
			...
			// vendor_customer
			$this->vendor_customer->ViewValue = $this->vendor_customer->CurrentValue;
			$this->vendor_customer->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// qty
			$CurrentValue = $this->qty->SumValue;
			$ViewValue = &$this->qty->SumViewValue;
			...
			// vendor_customer
			$CurrentValue = $this->vendor_customer->CurrentValue;
			$ViewValue = &$this->vendor_customer->ViewValue;
			$ViewAttrs = &$this->vendor_customer->ViewAttrs;
			$CellAttrs = &$this->vendor_customer->CellAttrs;
			$HrefValue = &$this->vendor_customer->HrefValue;
			$LinkAttrs = &$this->vendor_customer->LinkAttrs;
			$this->Cell_Rendered($this->vendor_customer, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
	...
	function SetupFieldCount() {
		...
		if ($this->vendor_customer->Visible) $this->DtlColumnCount += 1;
	...
	function GetSort($options = array()) {
		...
		// Check for an Order parameter
			...
			$this->UpdateSort($this->vendor_customer, $bCtrl); // vendor_customer
...
// Create page object
if (!isset($r_transaksi_summary)) $r_transaksi_summary = new crr_transaksi_summary();
if (isset($Page)) $OldPage = $Page;
$Page = &$r_transaksi_summary;
...
// Create page object
var r_transaksi_summary = new ewr_Page("r_transaksi_summary");

// Page properties
r_transaksi_summary.PageID = "summary"; // Page ID
var EWR_PAGE_ID = r_transaksi_summary.PageID;

// Extend page with Chart_Rendering function
r_transaksi_summary.Chart_Rendering = 
...
r_transaksi_summary.Chart_Rendered = 
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
<?php include "r_transaksismrypager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php } ?>
...
<?php } ?>
<span data-class="tpb<?php echo $Page->GrpCount-1 ?>_r_transaksi"><?php echo $Page->PageBreakContent ?></span>
<?php } ?>
<?php if ($Page->Export <> "pdf") { ?>
...
<?php if ($Page->item_id->Visible) { ?>
	...
	<?php } else { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="item_id"><div class="r_transaksi_item_id"><span class="ewTableHeaderCaption"><?php echo $Page->item_id->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="item_id">
<?php if ($Page->SortUrl($Page->item_id) == "") { ?>
		<div class="ewTableHeaderBtn r_transaksi_item_id">
			<span class="ewTableHeaderCaption"><?php echo $Page->item_id->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_transaksi_item_id" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->item_id) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->item_id->FldCaption() ?></span>
...
<?php if ($Page->item_nama->Visible) { ?>
	...
	<?php } else { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="item_nama"><div class="r_transaksi_item_nama"><span class="ewTableHeaderCaption"><?php echo $Page->item_nama->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="item_nama">
<?php if ($Page->SortUrl($Page->item_nama) == "") { ?>
		<div class="ewTableHeaderBtn r_transaksi_item_nama">
			<span class="ewTableHeaderCaption"><?php echo $Page->item_nama->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_transaksi_item_nama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->item_nama) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->item_nama->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="tgl"><div class="r_transaksi_tgl"><span class="ewTableHeaderCaption"><?php echo $Page->tgl->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="tgl">
<?php if ($Page->SortUrl($Page->tgl) == "") { ?>
		<div class="ewTableHeaderBtn r_transaksi_tgl">
			<span class="ewTableHeaderCaption"><?php echo $Page->tgl->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_transaksi_tgl" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->tgl) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->tgl->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="jenis"><div class="r_transaksi_jenis"><span class="ewTableHeaderCaption"><?php echo $Page->jenis->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="jenis">
<?php if ($Page->SortUrl($Page->jenis) == "") { ?>
		<div class="ewTableHeaderBtn r_transaksi_jenis">
			<span class="ewTableHeaderCaption"><?php echo $Page->jenis->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_transaksi_jenis" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->jenis) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->jenis->FldCaption() ?></span>
...
<?php } ?>
<?php } ?>
<?php if ($Page->vendor_customer->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="vendor_customer"><div class="r_transaksi_vendor_customer"><span class="ewTableHeaderCaption"><?php echo $Page->vendor_customer->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="vendor_customer">
<?php if ($Page->SortUrl($Page->vendor_customer) == "") { ?>
		<div class="ewTableHeaderBtn r_transaksi_vendor_customer">
			<span class="ewTableHeaderCaption"><?php echo $Page->vendor_customer->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_transaksi_vendor_customer" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->vendor_customer) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->vendor_customer->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->vendor_customer->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->vendor_customer->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="qty"><div class="r_transaksi_qty" style="text-align: right;"><span class="ewTableHeaderCaption"><?php echo $Page->qty->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="qty">
<?php if ($Page->SortUrl($Page->qty) == "") { ?>
		<div class="ewTableHeaderBtn r_transaksi_qty" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->qty->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_transaksi_qty" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->qty) ?>',2);" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->qty->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="satuan_nama"><div class="r_transaksi_satuan_nama"><span class="ewTableHeaderCaption"><?php echo $Page->satuan_nama->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="satuan_nama">
<?php if ($Page->SortUrl($Page->satuan_nama) == "") { ?>
		<div class="ewTableHeaderBtn r_transaksi_satuan_nama">
			<span class="ewTableHeaderCaption"><?php echo $Page->satuan_nama->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_transaksi_satuan_nama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->satuan_nama) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->satuan_nama->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="harga"><div class="r_transaksi_harga" style="text-align: right;"><span class="ewTableHeaderCaption"><?php echo $Page->harga->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="harga">
<?php if ($Page->SortUrl($Page->harga) == "") { ?>
		<div class="ewTableHeaderBtn r_transaksi_harga" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->harga->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_transaksi_harga" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->harga) ?>',2);" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->harga->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="sub_total"><div class="r_transaksi_sub_total" style="text-align: right;"><span class="ewTableHeaderCaption"><?php echo $Page->sub_total->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="sub_total">
<?php if ($Page->SortUrl($Page->sub_total) == "") { ?>
		<div class="ewTableHeaderBtn r_transaksi_sub_total" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->sub_total->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_transaksi_sub_total" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->sub_total) ?>',2);" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->sub_total->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
		<span class="ewSummaryCaption r_transaksi_item_id"><span class="ewTableHeaderCaption"><?php echo $Page->item_id->FldCaption() ?></span></span>
<?php } else { ?>
	<?php if ($Page->SortUrl($Page->item_id) == "") { ?>
		<span class="ewSummaryCaption r_transaksi_item_id">
			<span class="ewTableHeaderCaption"><?php echo $Page->item_id->FldCaption() ?></span>
		</span>
	<?php } else { ?>
		<span class="ewTableHeaderBtn ewPointer ewSummaryCaption r_transaksi_item_id" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->item_id) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->item_id->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->item_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->item_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</span>
	<?php } ?>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_r_transaksi_item_id"<?php echo $Page->item_id->ViewAttributes() ?>><?php echo $Page->item_id->GroupViewValue ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
		<span class="ewSummaryCaption r_transaksi_item_nama"><span class="ewTableHeaderCaption"><?php echo $Page->item_nama->FldCaption() ?></span></span>
<?php } else { ?>
	<?php if ($Page->SortUrl($Page->item_nama) == "") { ?>
		<span class="ewSummaryCaption r_transaksi_item_nama">
			<span class="ewTableHeaderCaption"><?php echo $Page->item_nama->FldCaption() ?></span>
		</span>
	<?php } else { ?>
		<span class="ewTableHeaderBtn ewPointer ewSummaryCaption r_transaksi_item_nama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->item_nama) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->item_nama->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->item_nama->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->item_nama->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</span>
	<?php } ?>
<?php } ?>
		<?php echo $ReportLanguage->Phrase("SummaryColon") ?>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_r_transaksi_item_nama"<?php echo $Page->item_nama->ViewAttributes() ?>><?php echo $Page->item_nama->GroupViewValue ?></span>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_r_transaksi_item_id"<?php echo $Page->item_id->ViewAttributes() ?>><?php echo $Page->item_id->GroupViewValue ?></span></td>
	<?php } ?>
<?php } ?>
<?php if ($Page->item_nama->Visible) { ?>
	<?php if ($Page->item_nama->ShowGroupHeaderAsRow) { ?>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_r_transaksi_item_nama"<?php echo $Page->item_nama->ViewAttributes() ?>><?php echo $Page->item_nama->GroupViewValue ?></span></td>
	<?php } ?>
<?php } ?>
<?php if ($Page->tgl->Visible) { ?>
		<td data-field="tgl"<?php echo $Page->tgl->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->RecCount ?>_r_transaksi_tgl"<?php echo $Page->tgl->ViewAttributes() ?>><?php echo $Page->tgl->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->jenis->Visible) { ?>
		<td data-field="jenis"<?php echo $Page->jenis->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->RecCount ?>_r_transaksi_jenis"<?php echo $Page->jenis->ViewAttributes() ?>><?php echo $Page->jenis->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->vendor_customer->Visible) { ?>
		<td data-field="vendor_customer"<?php echo $Page->vendor_customer->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->RecCount ?>_r_transaksi_vendor_customer"<?php echo $Page->vendor_customer->ViewAttributes() ?>><?php echo $Page->vendor_customer->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->qty->Visible) { ?>
		<td data-field="qty"<?php echo $Page->qty->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->RecCount ?>_r_transaksi_qty"<?php echo $Page->qty->ViewAttributes() ?>><?php echo $Page->qty->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->satuan_nama->Visible) { ?>
		<td data-field="satuan_nama"<?php echo $Page->satuan_nama->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->RecCount ?>_r_transaksi_satuan_nama"<?php echo $Page->satuan_nama->ViewAttributes() ?>><?php echo $Page->satuan_nama->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->harga->Visible) { ?>
		<td data-field="harga"<?php echo $Page->harga->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->RecCount ?>_r_transaksi_harga"<?php echo $Page->harga->ViewAttributes() ?>><?php echo $Page->harga->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->sub_total->Visible) { ?>
		<td data-field="sub_total"<?php echo $Page->sub_total->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->RecCount ?>_r_transaksi_sub_total"<?php echo $Page->sub_total->ViewAttributes() ?>><?php echo $Page->sub_total->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
...
<?php
			...
			$Page->qty->Count = $Page->Cnt[1][4];
			$Page->qty->SumValue = $Page->Smry[1][4]; // Load SUM
			$Page->sub_total->Count = $Page->Cnt[1][7];
			$Page->sub_total->SumValue = $Page->Smry[1][7]; // Load SUM
			$Page->ResetAttrs();
			$Page->RowType = EWR_ROWTYPE_TOTAL;
			$Page->RowTotalType = EWR_ROWTOTAL_GROUP;
...
<?php } ?>
<?php if ($Page->vendor_customer->Visible) { ?>
		<td data-field="vendor_customer"<?php echo $Page->item_id->CellAttributes() ?>></td>
<?php } ?>
<?php if ($Page->qty->Visible) { ?>
		<td data-field="qty"<?php echo $Page->item_id->CellAttributes() ?>><span class="ewAggregateCaption"><?php echo $ReportLanguage->Phrase("RptSum") ?></span><?php echo $ReportLanguage->Phrase("AggregateEqual") ?><span class="ewAggregateValue"><span data-class="tpgs<?php echo $Page->GrpCount ?>_r_transaksi_qty"<?php echo $Page->qty->ViewAttributes() ?>><?php echo $Page->qty->SumViewValue ?></span></span></td>
<?php } ?>
<?php if ($Page->satuan_nama->Visible) { ?>
		<td data-field="satuan_nama"<?php echo $Page->item_id->CellAttributes() ?>></td>
<?php } ?>
<?php if ($Page->harga->Visible) { ?>
		<td data-field="harga"<?php echo $Page->item_id->CellAttributes() ?>></td>
<?php } ?>
<?php if ($Page->sub_total->Visible) { ?>
		<td data-field="sub_total"<?php echo $Page->item_id->CellAttributes() ?>><span class="ewAggregateCaption"><?php echo $ReportLanguage->Phrase("RptSum") ?></span><?php echo $ReportLanguage->Phrase("AggregateEqual") ?><span class="ewAggregateValue"><span data-class="tpgs<?php echo $Page->GrpCount ?>_r_transaksi_sub_total"<?php echo $Page->sub_total->ViewAttributes() ?>><?php echo $Page->sub_total->SumViewValue ?></span></span></td>
<?php } ?>
	</tr>
<?php } else { ?>
	<tr<?php echo $Page->RowAttributes(); ?>>
<?php if ($Page->GrpColumnCount + $Page->DtlColumnCount > 0) { ?>
		<td colspan="<?php echo ($Page->GrpColumnCount + $Page->DtlColumnCount) ?>"<?php echo $Page->sub_total->CellAttributes() ?>><?php echo str_replace(array("%v", "%c"), array($Page->item_id->GroupViewValue, $Page->item_id->FldCaption()), $ReportLanguage->Phrase("RptSumHead")) ?> <span class="ewDirLtr">(<?php echo ewr_FormatNumber($Page->Cnt[1][0],0,-2,-2,-2) ?><?php echo $ReportLanguage->Phrase("RptDtlRec") ?>)</span></td>
<?php } ?>
	</tr>
	<tr<?php echo $Page->RowAttributes(); ?>>
...
<?php } ?>
<?php if ($Page->qty->Visible) { ?>
		<td data-field="qty"<?php echo $Page->sub_total->CellAttributes() ?>>
<span data-class="tpgs<?php echo $Page->GrpCount ?>_r_transaksi_qty"<?php echo $Page->qty->ViewAttributes() ?>><?php echo $Page->qty->SumViewValue ?></span></td>
<?php } ?>
<?php if ($Page->satuan_nama->Visible) { ?>
		<td data-field="satuan_nama"<?php echo $Page->item_id->CellAttributes() ?>>&nbsp;</td>
<?php } ?>
...
<?php if ($Page->sub_total->Visible) { ?>
		<td data-field="sub_total"<?php echo $Page->sub_total->CellAttributes() ?>>
<span data-class="tpgs<?php echo $Page->GrpCount ?>_r_transaksi_sub_total"<?php echo $Page->sub_total->ViewAttributes() ?>><?php echo $Page->sub_total->SumViewValue ?></span></td>
<?php } ?>
	</tr>
<?php } ?>
<?php
...
<tfoot>
<?php
	$Page->qty->Count = $Page->GrandCnt[4];
	$Page->qty->SumValue = $Page->GrandSmry[4]; // Load SUM
	$Page->sub_total->Count = $Page->GrandCnt[7];
	$Page->sub_total->SumValue = $Page->GrandSmry[7]; // Load SUM
	$Page->ResetAttrs();
	$Page->RowType = EWR_ROWTYPE_TOTAL;
...
<?php } ?>
<?php if ($Page->vendor_customer->Visible) { ?>
		<td data-field="vendor_customer"<?php echo $Page->vendor_customer->CellAttributes() ?>></td>
<?php } ?>
<?php if ($Page->qty->Visible) { ?>
		<td data-field="qty"<?php echo $Page->qty->CellAttributes() ?>><?php echo $ReportLanguage->Phrase("RptSum") ?>=<span data-class="tpts_r_transaksi_qty"<?php echo $Page->qty->ViewAttributes() ?>><?php echo $Page->qty->SumViewValue ?></span></td>
<?php } ?>
<?php if ($Page->satuan_nama->Visible) { ?>
...
<?php } ?>
<?php if ($Page->sub_total->Visible) { ?>
		<td data-field="sub_total"<?php echo $Page->sub_total->CellAttributes() ?>><?php echo $ReportLanguage->Phrase("RptSum") ?>=<span data-class="tpts_r_transaksi_sub_total"<?php echo $Page->sub_total->ViewAttributes() ?>><?php echo $Page->sub_total->SumViewValue ?></span></td>
<?php } ?>
	</tr>
<?php } else { ?>
...
<?php if ($Page->qty->Visible) { ?>
		<td data-field="qty"<?php echo $Page->qty->CellAttributes() ?>>
<span data-class="tpts_r_transaksi_qty"<?php echo $Page->qty->ViewAttributes() ?>><?php echo $Page->qty->SumViewValue ?></span></td>
<?php } ?>
<?php if ($Page->satuan_nama->Visible) { ?>
		<td data-field="satuan_nama"<?php echo $Page->satuan_nama->CellAttributes() ?>>&nbsp;</td>
<?php } ?>
...
<?php if ($Page->sub_total->Visible) { ?>
		<td data-field="sub_total"<?php echo $Page->sub_total->CellAttributes() ?>>
<span data-class="tpts_r_transaksi_sub_total"<?php echo $Page->sub_total->ViewAttributes() ?>><?php echo $Page->sub_total->SumViewValue ?></span></td>
<?php } ?>
	</tr>
<?php } ?>
	</tfoot>
