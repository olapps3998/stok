...
$t_07jual_detail_list = NULL; // Initialize page object first

class ct_07jual_detail_list extends ct_07jual_detail {
	...
	// Page object name
	var $PageObjName = 't_07jual_detail_list';

	// Grid form hidden field names
	var $FormName = 'ft_07jual_detaillist';
	...
	function __construct() {
		...
		$this->AddUrl = "t_07jual_detailadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "t_07jual_detaildelete.php";
		$this->MultiUpdateUrl = "t_07jual_detailupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption ft_07jual_detaillistsrch";
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "t_06jual") {
			global $t_06jual;
			$rsmaster = $t_06jual->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("t_06juallist.php"); // Return to master page
			} else {
				$t_06jual->LoadListRowValues($rsmaster);
				$t_06jual->RowType = EW_ROWTYPE_MASTER; // Master row
				$t_06jual->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function ClearInlineMode() {
		$this->setKey("jual_detail_id", ""); // Clear inline edit key
	...
	function InlineEditMode() {
		...
		if ($bInlineEdit) {
			if ($this->LoadRow()) {
				$this->setKey("jual_detail_id", $this->jual_detail_id->CurrentValue); // Set up inline edit key
	...
	function CheckInlineEditKey() {
		...
		if (strval($this->getKey("jual_detail_id")) <> strval($this->jual_detail_id->CurrentValue))
			return FALSE;
		return TRUE;
	}
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->jual_detail_id->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->jual_detail_id->FormValue))
				return FALSE;
		}
		return TRUE;
	}
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->jual_detail_id->CurrentValue;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentAction == "edit" && $this->RowType == EW_ROWTYPE_EDIT) { // Inline-Edit
			...
			$oListOpt->Body .= "<input type=\"hidden\" name=\"k" . $this->RowIndex . "_key\" id=\"k" . $this->RowIndex . "_key\" value=\"" . ew_HtmlEncode($this->jual_detail_id->CurrentValue) . "\">";
			return;
		}
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" value=\"" . ew_HtmlEncode($this->jual_detail_id->CurrentValue) . "\" onclick='ew_ClickMultiCheckbox(event);'>";
		if ($this->CurrentAction == "gridedit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->jual_detail_id->CurrentValue . "\">";
		}
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewAction ewMultiDelete\" title=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("DeleteSelectedLink")) . "\" href=\"\" onclick=\"ew_SubmitAction(event,{f:document.ft_07jual_detaillist,url:'" . $this->MultiDeleteUrl . "'});return false;\">" . $Language->Phrase("DeleteSelectedLink") . "</a>";
		$item->Visible = ($Security->CanDelete());
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"ft_07jual_detaillistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = FALSE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"ft_07jual_detaillistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = FALSE;
		...
		if ($this->CurrentAction <> "gridadd" && $this->CurrentAction <> "gridedit") { // Not grid add/edit mode
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.ft_07jual_detaillist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function LoadFormValues() {
		...
		if (!$this->jual_detail_id->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function ExportData() {
		...
		if (EW_EXPORT_MASTER_RECORD && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "t_06jual") {
			global $t_06jual;
			if (!isset($t_06jual)) $t_06jual = new ct_06jual;
			$rsmaster = $t_06jual->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				$ExportStyle = $Doc->Style;
				$Doc->SetStyle("v"); // Change to vertical
				if ($this->Export <> "csv" || EW_EXPORT_MASTER_RECORD_FOR_CSV) {
					$Doc->Table = &$t_06jual;
					$t_06jual->ExportDocument($Doc, $rsmaster, 1, 1);
					$Doc->ExportEmptyRow();
					$Doc->Table = &$this;
				}
...
// Create page object
if (!isset($t_07jual_detail_list)) $t_07jual_detail_list = new ct_07jual_detail_list();

// Page init
$t_07jual_detail_list->Page_Init();

// Page main
$t_07jual_detail_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t_07jual_detail_list->Page_Render();
?>
...
var CurrentForm = ft_07jual_detaillist = new ew_Form("ft_07jual_detaillist", "list");
ft_07jual_detaillist.FormKeyCountName = '<?php echo $t_07jual_detail_list->FormKeyCountName ?>';

// Validate form
ft_07jual_detaillist.Validate = function() {
...
// Check empty row
ft_07jual_detaillist.EmptyRow = function(infix) {
	var fobj = this.Form;
...
ft_07jual_detaillist.Form_CustomValidate = 
...
ft_07jual_detaillist.ValidateRequired = true;
<?php } else { ?>
ft_07jual_detaillist.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php if ($t_07jual_detail_list->TotalRecs > 0 && $t_07jual_detail_list->ExportOptions->Visible()) { ?>
<?php $t_07jual_detail_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($t_07jual_detail->Export == "") { ?>
...
<?php } ?>
<?php if (($t_07jual_detail->Export == "") || (EW_EXPORT_MASTER_RECORD && $t_07jual_detail->Export == "print")) { ?>
<?php
if ($t_07jual_detail_list->DbMasterFilter <> "" && $t_07jual_detail->getCurrentMasterTable() == "t_06jual") {
	if ($t_07jual_detail_list->MasterRecordExists) {
?>
<?php include_once "t_06jualmaster.php" ?>
<?php
	}
}
?>
...
if ($t_07jual_detail->CurrentAction == "gridadd") {
	...
	$t_07jual_detail_list->StartRec = 1;
	$t_07jual_detail_list->DisplayRecs = $t_07jual_detail->GridAddRowCount;
	$t_07jual_detail_list->TotalRecs = $t_07jual_detail_list->DisplayRecs;
	$t_07jual_detail_list->StopRec = $t_07jual_detail_list->DisplayRecs;
} else {
	$bSelectLimit = $t_07jual_detail_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($t_07jual_detail_list->TotalRecs <= 0)
			$t_07jual_detail_list->TotalRecs = $t_07jual_detail->SelectRecordCount();
	} else {
		if (!$t_07jual_detail_list->Recordset && ($t_07jual_detail_list->Recordset = $t_07jual_detail_list->LoadRecordset()))
			$t_07jual_detail_list->TotalRecs = $t_07jual_detail_list->Recordset->RecordCount();
	}
	$t_07jual_detail_list->StartRec = 1;
	if ($t_07jual_detail_list->DisplayRecs <= 0 || ($t_07jual_detail->Export <> "" && $t_07jual_detail->ExportAll)) // Display all records
		$t_07jual_detail_list->DisplayRecs = $t_07jual_detail_list->TotalRecs;
	if (!($t_07jual_detail->Export <> "" && $t_07jual_detail->ExportAll))
		$t_07jual_detail_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$t_07jual_detail_list->Recordset = $t_07jual_detail_list->LoadRecordset($t_07jual_detail_list->StartRec-1, $t_07jual_detail_list->DisplayRecs);

	// Set no record found message
	if ($t_07jual_detail->CurrentAction == "" && $t_07jual_detail_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$t_07jual_detail_list->setWarningMessage(ew_DeniedMsg());
		if ($t_07jual_detail_list->SearchWhere == "0=101")
			$t_07jual_detail_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$t_07jual_detail_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
}
$t_07jual_detail_list->RenderOtherOptions();
?>
<?php $t_07jual_detail_list->ShowPageHeader(); ?>
<?php
$t_07jual_detail_list->ShowMessage();
?>
<?php if ($t_07jual_detail_list->TotalRecs > 0 || $t_07jual_detail->CurrentAction <> "") { ?>
<div class="panel panel-default ewGrid t_07jual_detail">
<?php if ($t_07jual_detail->Export == "") { ?>
<div class="panel-heading ewGridUpperPanel">
<?php if ($t_07jual_detail->CurrentAction <> "gridadd" && $t_07jual_detail->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($t_07jual_detail_list->Pager)) $t_07jual_detail_list->Pager = new cPrevNextPager($t_07jual_detail_list->StartRec, $t_07jual_detail_list->DisplayRecs, $t_07jual_detail_list->TotalRecs) ?>
<?php if ($t_07jual_detail_list->Pager->RecordCount > 0 && $t_07jual_detail_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($t_07jual_detail_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $t_07jual_detail_list->PageUrl() ?>start=<?php echo $t_07jual_detail_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($t_07jual_detail_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $t_07jual_detail_list->PageUrl() ?>start=<?php echo $t_07jual_detail_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $t_07jual_detail_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($t_07jual_detail_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $t_07jual_detail_list->PageUrl() ?>start=<?php echo $t_07jual_detail_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($t_07jual_detail_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $t_07jual_detail_list->PageUrl() ?>start=<?php echo $t_07jual_detail_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $t_07jual_detail_list->Pager->PageCount ?></span>
</div>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $t_07jual_detail_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $t_07jual_detail_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $t_07jual_detail_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($t_07jual_detail_list->TotalRecs > 0 && (!EW_AUTO_HIDE_PAGE_SIZE_SELECTOR || $t_07jual_detail_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="t_07jual_detail">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="10"<?php if ($t_07jual_detail_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="20"<?php if ($t_07jual_detail_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($t_07jual_detail_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="100"<?php if ($t_07jual_detail_list->DisplayRecs == 100) { ?> selected<?php } ?>>100</option>
<option value="200"<?php if ($t_07jual_detail_list->DisplayRecs == 200) { ?> selected<?php } ?>>200</option>
<option value="ALL"<?php if ($t_07jual_detail->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($t_07jual_detail_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="ft_07jual_detaillist" id="ft_07jual_detaillist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t_07jual_detail_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $t_07jual_detail_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="t_07jual_detail">
<?php if ($t_07jual_detail->getCurrentMasterTable() == "t_06jual" && $t_07jual_detail->CurrentAction <> "") { ?>
...
<?php if ($t_07jual_detail_list->TotalRecs > 0 || $t_07jual_detail->CurrentAction == "add" || $t_07jual_detail->CurrentAction == "copy" || $t_07jual_detail->CurrentAction == "gridedit") { ?>
<table id="tbl_t_07jual_detaillist" class="table ewTable">
...
// Header row
$t_07jual_detail_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$t_07jual_detail_list->RenderListOptions();

// Render list options (header, left)
$t_07jual_detail_list->ListOptions->Render("header", "left");
?>
<?php if ($t_07jual_detail->tgl_kirim->Visible) { // tgl_kirim ?>
	...
	<?php } else { ?>
		<th data-name="tgl_kirim"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t_07jual_detail->SortUrl($t_07jual_detail->tgl_kirim) ?>',2);"><div id="elh_t_07jual_detail_tgl_kirim" class="t_07jual_detail_tgl_kirim">
...
<?php if ($t_07jual_detail->item_id->Visible) { // item_id ?>
	...
	<?php } else { ?>
		<th data-name="item_id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t_07jual_detail->SortUrl($t_07jual_detail->item_id) ?>',2);"><div id="elh_t_07jual_detail_item_id" class="t_07jual_detail_item_id">
...
<?php if ($t_07jual_detail->qty->Visible) { // qty ?>
	...
	<?php } else { ?>
		<th data-name="qty"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t_07jual_detail->SortUrl($t_07jual_detail->qty) ?>',2);"><div id="elh_t_07jual_detail_qty" class="t_07jual_detail_qty">
...
<?php if ($t_07jual_detail->satuan_id->Visible) { // satuan_id ?>
	...
	<?php } else { ?>
		<th data-name="satuan_id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t_07jual_detail->SortUrl($t_07jual_detail->satuan_id) ?>',2);"><div id="elh_t_07jual_detail_satuan_id" class="t_07jual_detail_satuan_id">
...
<?php if ($t_07jual_detail->harga->Visible) { // harga ?>
	...
	<?php } else { ?>
		<th data-name="harga"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t_07jual_detail->SortUrl($t_07jual_detail->harga) ?>',2);"><div id="elh_t_07jual_detail_harga" class="t_07jual_detail_harga">
...
<?php if ($t_07jual_detail->sub_total->Visible) { // sub_total ?>
	...
	<?php } else { ?>
		<th data-name="sub_total"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t_07jual_detail->SortUrl($t_07jual_detail->sub_total) ?>',2);"><div id="elh_t_07jual_detail_sub_total" class="t_07jual_detail_sub_total">
...
$t_07jual_detail_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
	if ($t_07jual_detail->CurrentAction == "add" || $t_07jual_detail->CurrentAction == "copy") {
		$t_07jual_detail_list->RowIndex = 0;
		$t_07jual_detail_list->KeyCount = $t_07jual_detail_list->RowIndex;
		if ($t_07jual_detail->CurrentAction == "copy" && !$t_07jual_detail_list->LoadRow())
				$t_07jual_detail->CurrentAction = "add";
		if ($t_07jual_detail->CurrentAction == "add")
			$t_07jual_detail_list->LoadDefaultValues();
		if ($t_07jual_detail->EventCancelled) // Insert failed
			$t_07jual_detail_list->RestoreFormValues(); // Restore form values

		// Set row properties
		$t_07jual_detail->ResetAttrs();
		$t_07jual_detail->RowAttrs = array_merge($t_07jual_detail->RowAttrs, array('data-rowindex'=>0, 'id'=>'r0_t_07jual_detail', 'data-rowtype'=>EW_ROWTYPE_ADD));
		$t_07jual_detail->RowType = EW_ROWTYPE_ADD;

		// Render row
		$t_07jual_detail_list->RenderRow();

		// Render list options
		$t_07jual_detail_list->RenderListOptions();
		$t_07jual_detail_list->StartRowCnt = 0;
?>
	<tr<?php echo $t_07jual_detail->RowAttributes() ?>>
<?php

// Render list options (body, left)
$t_07jual_detail_list->ListOptions->Render("body", "left", $t_07jual_detail_list->RowCnt);
?>
	<?php if ($t_07jual_detail->tgl_kirim->Visible) { // tgl_kirim ?>
		<td data-name="tgl_kirim">
<span id="el<?php echo $t_07jual_detail_list->RowCnt ?>_t_07jual_detail_tgl_kirim" class="form-group t_07jual_detail_tgl_kirim">
<input type="text" data-table="t_07jual_detail" data-field="x_tgl_kirim" data-format="7" name="x<?php echo $t_07jual_detail_list->RowIndex ?>_tgl_kirim" id="x<?php echo $t_07jual_detail_list->RowIndex ?>_tgl_kirim" placeholder="<?php echo ew_HtmlEncode($t_07jual_detail->tgl_kirim->getPlaceHolder()) ?>" value="<?php echo $t_07jual_detail->tgl_kirim->EditValue ?>"<?php echo $t_07jual_detail->tgl_kirim->EditAttributes() ?>>
...
<script type="text/javascript">
ew_CreateCalendar("ft_07jual_detaillist", "x<?php echo $t_07jual_detail_list->RowIndex ?>_tgl_kirim", 7);
</script>
<?php } ?>
</span>
<input type="hidden" data-table="t_07jual_detail" data-field="x_tgl_kirim" name="o<?php echo $t_07jual_detail_list->RowIndex ?>_tgl_kirim" id="o<?php echo $t_07jual_detail_list->RowIndex ?>_tgl_kirim" value="<?php echo ew_HtmlEncode($t_07jual_detail->tgl_kirim->OldValue) ?>">
</td>
	<?php } ?>
	<?php if ($t_07jual_detail->item_id->Visible) { // item_id ?>
...
?>
<span id="as_x<?php echo $t_07jual_detail_list->RowIndex ?>_item_id" style="white-space: nowrap; z-index: <?php echo (9000 - $t_07jual_detail_list->RowCnt * 10) ?>">
	<input type="text" name="sv_x<?php echo $t_07jual_detail_list->RowIndex ?>_item_id" id="sv_x<?php echo $t_07jual_detail_list->RowIndex ?>_item_id" value="<?php echo $t_07jual_detail->item_id->EditValue ?>" placeholder="<?php echo ew_HtmlEncode($t_07jual_detail->item_id->getPlaceHolder()) ?>" data-placeholder="<?php echo ew_HtmlEncode($t_07jual_detail->item_id->getPlaceHolder()) ?>"<?php echo $t_07jual_detail->item_id->EditAttributes() ?>>
</span>
<input type="hidden" data-table="t_07jual_detail" data-field="x_item_id" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $t_07jual_detail->item_id->DisplayValueSeparatorAttribute() ?>" name="x<?php echo $t_07jual_detail_list->RowIndex ?>_item_id" id="x<?php echo $t_07jual_detail_list->RowIndex ?>_item_id" value="<?php echo ew_HtmlEncode($t_07jual_detail->item_id->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<input type="hidden" name="q_x<?php echo $t_07jual_detail_list->RowIndex ?>_item_id" id="q_x<?php echo $t_07jual_detail_list->RowIndex ?>_item_id" value="<?php echo $t_07jual_detail->item_id->LookupFilterQuery(true) ?>">
<script type="text/javascript">
ft_07jual_detaillist.CreateAutoSuggest({"id":"x<?php echo $t_07jual_detail_list->RowIndex ?>_item_id","forceSelect":true});
</script>
...
<input type="hidden" data-table="t_07jual_detail" data-field="x_item_id" name="o<?php echo $t_07jual_detail_list->RowIndex ?>_item_id" id="o<?php echo $t_07jual_detail_list->RowIndex ?>_item_id" value="<?php echo ew_HtmlEncode($t_07jual_detail->item_id->OldValue) ?>">
	...
	<?php if ($t_07jual_detail->qty->Visible) { // qty ?>
		<td data-name="qty">
<span id="el<?php echo $t_07jual_detail_list->RowCnt ?>_t_07jual_detail_qty" class="form-group t_07jual_detail_qty">
<input type="text" data-table="t_07jual_detail" data-field="x_qty" name="x<?php echo $t_07jual_detail_list->RowIndex ?>_qty" id="x<?php echo $t_07jual_detail_list->RowIndex ?>_qty" placeholder="<?php echo ew_HtmlEncode($t_07jual_detail->qty->getPlaceHolder()) ?>" value="<?php echo $t_07jual_detail->qty->EditValue ?>"<?php echo $t_07jual_detail->qty->EditAttributes() ?>>
</span>
<input type="hidden" data-table="t_07jual_detail" data-field="x_qty" name="o<?php echo $t_07jual_detail_list->RowIndex ?>_qty" id="o<?php echo $t_07jual_detail_list->RowIndex ?>_qty" value="<?php echo ew_HtmlEncode($t_07jual_detail->qty->OldValue) ?>">
</td>
	<?php } ?>
	<?php if ($t_07jual_detail->satuan_id->Visible) { // satuan_id ?>
		<td data-name="satuan_id">
<span id="el<?php echo $t_07jual_detail_list->RowCnt ?>_t_07jual_detail_satuan_id" class="form-group t_07jual_detail_satuan_id">
<?php
$wrkonchange = trim(" " . @$t_07jual_detail->satuan_id->EditAttrs["onchange"]);
if ($wrkonchange <> "") $wrkonchange = " onchange=\"" . ew_JsEncode2($wrkonchange) . "\"";
$t_07jual_detail->satuan_id->EditAttrs["onchange"] = "";
?>
<span id="as_x<?php echo $t_07jual_detail_list->RowIndex ?>_satuan_id" style="white-space: nowrap; z-index: <?php echo (9000 - $t_07jual_detail_list->RowCnt * 10) ?>">
	<input type="text" name="sv_x<?php echo $t_07jual_detail_list->RowIndex ?>_satuan_id" id="sv_x<?php echo $t_07jual_detail_list->RowIndex ?>_satuan_id" value="<?php echo $t_07jual_detail->satuan_id->EditValue ?>" placeholder="<?php echo ew_HtmlEncode($t_07jual_detail->satuan_id->getPlaceHolder()) ?>" data-placeholder="<?php echo ew_HtmlEncode($t_07jual_detail->satuan_id->getPlaceHolder()) ?>"<?php echo $t_07jual_detail->satuan_id->EditAttributes() ?>>
</span>
<input type="hidden" data-table="t_07jual_detail" data-field="x_satuan_id" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $t_07jual_detail->satuan_id->DisplayValueSeparatorAttribute() ?>" name="x<?php echo $t_07jual_detail_list->RowIndex ?>_satuan_id" id="x<?php echo $t_07jual_detail_list->RowIndex ?>_satuan_id" value="<?php echo ew_HtmlEncode($t_07jual_detail->satuan_id->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<input type="hidden" name="q_x<?php echo $t_07jual_detail_list->RowIndex ?>_satuan_id" id="q_x<?php echo $t_07jual_detail_list->RowIndex ?>_satuan_id" value="<?php echo $t_07jual_detail->satuan_id->LookupFilterQuery(true) ?>">
<script type="text/javascript">
ft_07jual_detaillist.CreateAutoSuggest({"id":"x<?php echo $t_07jual_detail_list->RowIndex ?>_satuan_id","forceSelect":true});
</script>
...
<input type="hidden" data-table="t_07jual_detail" data-field="x_satuan_id" name="o<?php echo $t_07jual_detail_list->RowIndex ?>_satuan_id" id="o<?php echo $t_07jual_detail_list->RowIndex ?>_satuan_id" value="<?php echo ew_HtmlEncode($t_07jual_detail->satuan_id->OldValue) ?>">
	...
	<?php if ($t_07jual_detail->harga->Visible) { // harga ?>
		<td data-name="harga">
<span id="el<?php echo $t_07jual_detail_list->RowCnt ?>_t_07jual_detail_harga" class="form-group t_07jual_detail_harga">
<input type="text" data-table="t_07jual_detail" data-field="x_harga" name="x<?php echo $t_07jual_detail_list->RowIndex ?>_harga" id="x<?php echo $t_07jual_detail_list->RowIndex ?>_harga" placeholder="<?php echo ew_HtmlEncode($t_07jual_detail->harga->getPlaceHolder()) ?>" value="<?php echo $t_07jual_detail->harga->EditValue ?>"<?php echo $t_07jual_detail->harga->EditAttributes() ?>>
</span>
<input type="hidden" data-table="t_07jual_detail" data-field="x_harga" name="o<?php echo $t_07jual_detail_list->RowIndex ?>_harga" id="o<?php echo $t_07jual_detail_list->RowIndex ?>_harga" value="<?php echo ew_HtmlEncode($t_07jual_detail->harga->OldValue) ?>">
</td>
	<?php } ?>
	<?php if ($t_07jual_detail->sub_total->Visible) { // sub_total ?>
		<td data-name="sub_total">
<span id="el<?php echo $t_07jual_detail_list->RowCnt ?>_t_07jual_detail_sub_total" class="form-group t_07jual_detail_sub_total">
<input type="text" data-table="t_07jual_detail" data-field="x_sub_total" name="x<?php echo $t_07jual_detail_list->RowIndex ?>_sub_total" id="x<?php echo $t_07jual_detail_list->RowIndex ?>_sub_total" placeholder="<?php echo ew_HtmlEncode($t_07jual_detail->sub_total->getPlaceHolder()) ?>" value="<?php echo $t_07jual_detail->sub_total->EditValue ?>"<?php echo $t_07jual_detail->sub_total->EditAttributes() ?>>
</span>
<input type="hidden" data-table="t_07jual_detail" data-field="x_sub_total" name="o<?php echo $t_07jual_detail_list->RowIndex ?>_sub_total" id="o<?php echo $t_07jual_detail_list->RowIndex ?>_sub_total" value="<?php echo ew_HtmlEncode($t_07jual_detail->sub_total->OldValue) ?>">
</td>
	<?php } ?>
<?php

// Render list options (body, right)
$t_07jual_detail_list->ListOptions->Render("body", "right", $t_07jual_detail_list->RowCnt);
?>
<script type="text/javascript">
ft_07jual_detaillist.UpdateOpts(<?php echo $t_07jual_detail_list->RowIndex ?>);
</script>
	</tr>
<?php
}
?>
<?php
if ($t_07jual_detail->ExportAll && $t_07jual_detail->Export <> "") {
	$t_07jual_detail_list->StopRec = $t_07jual_detail_list->TotalRecs;
} else {

	// Set the last record to display
	if ($t_07jual_detail_list->TotalRecs > $t_07jual_detail_list->StartRec + $t_07jual_detail_list->DisplayRecs - 1)
		$t_07jual_detail_list->StopRec = $t_07jual_detail_list->StartRec + $t_07jual_detail_list->DisplayRecs - 1;
	else
		$t_07jual_detail_list->StopRec = $t_07jual_detail_list->TotalRecs;
}
...
if ($objForm) {
	$objForm->Index = -1;
	if ($objForm->HasValue($t_07jual_detail_list->FormKeyCountName) && ($t_07jual_detail->CurrentAction == "gridadd" || $t_07jual_detail->CurrentAction == "gridedit" || $t_07jual_detail->CurrentAction == "F")) {
		$t_07jual_detail_list->KeyCount = $objForm->GetValue($t_07jual_detail_list->FormKeyCountName);
		$t_07jual_detail_list->StopRec = $t_07jual_detail_list->StartRec + $t_07jual_detail_list->KeyCount - 1;
	}
}
$t_07jual_detail_list->RecCnt = $t_07jual_detail_list->StartRec - 1;
if ($t_07jual_detail_list->Recordset && !$t_07jual_detail_list->Recordset->EOF) {
	$t_07jual_detail_list->Recordset->MoveFirst();
	$bSelectLimit = $t_07jual_detail_list->UseSelectLimit;
	if (!$bSelectLimit && $t_07jual_detail_list->StartRec > 1)
		$t_07jual_detail_list->Recordset->Move($t_07jual_detail_list->StartRec - 1);
} elseif (!$t_07jual_detail->AllowAddDeleteRow && $t_07jual_detail_list->StopRec == 0) {
	$t_07jual_detail_list->StopRec = $t_07jual_detail->GridAddRowCount;
}
...
$t_07jual_detail->ResetAttrs();
$t_07jual_detail_list->RenderRow();
$t_07jual_detail_list->EditRowCnt = 0;
if ($t_07jual_detail->CurrentAction == "edit")
	$t_07jual_detail_list->RowIndex = 1;
if ($t_07jual_detail->CurrentAction == "gridadd")
	$t_07jual_detail_list->RowIndex = 0;
if ($t_07jual_detail->CurrentAction == "gridedit")
	$t_07jual_detail_list->RowIndex = 0;
while ($t_07jual_detail_list->RecCnt < $t_07jual_detail_list->StopRec) {
	$t_07jual_detail_list->RecCnt++;
	if (intval($t_07jual_detail_list->RecCnt) >= intval($t_07jual_detail_list->StartRec)) {
		$t_07jual_detail_list->RowCnt++;
		if ($t_07jual_detail->CurrentAction == "gridadd" || $t_07jual_detail->CurrentAction == "gridedit" || $t_07jual_detail->CurrentAction == "F") {
			$t_07jual_detail_list->RowIndex++;
			$objForm->Index = $t_07jual_detail_list->RowIndex;
			if ($objForm->HasValue($t_07jual_detail_list->FormActionName))
				$t_07jual_detail_list->RowAction = strval($objForm->GetValue($t_07jual_detail_list->FormActionName));
			elseif ($t_07jual_detail->CurrentAction == "gridadd")
				$t_07jual_detail_list->RowAction = "insert";
			else
				$t_07jual_detail_list->RowAction = "";
		}
		...
		if ($t_07jual_detail->CurrentAction == "gridadd") {
			$t_07jual_detail_list->LoadDefaultValues(); // Load default values
		} else {
			$t_07jual_detail_list->LoadRowValues($t_07jual_detail_list->Recordset); // Load row values
		}
		...
		if ($t_07jual_detail->CurrentAction == "gridadd" && $t_07jual_detail->EventCancelled && !$objForm->HasValue("k_blankrow")) // Insert failed
			$t_07jual_detail_list->RestoreCurrentRowFormValues($t_07jual_detail_list->RowIndex); // Restore form values
		if ($t_07jual_detail->CurrentAction == "edit") {
			if ($t_07jual_detail_list->CheckInlineEditKey() && $t_07jual_detail_list->EditRowCnt == 0) { // Inline edit
		...
		if ($t_07jual_detail->CurrentAction == "gridedit") { // Grid edit
			...
			if ($t_07jual_detail_list->RowAction == "insert")
				$t_07jual_detail->RowType = EW_ROWTYPE_ADD; // Render add
			else
				$t_07jual_detail->RowType = EW_ROWTYPE_EDIT; // Render edit
		}
		if ($t_07jual_detail->CurrentAction == "edit" && $t_07jual_detail->RowType == EW_ROWTYPE_EDIT && $t_07jual_detail->EventCancelled) { // Update failed
			$objForm->Index = 1;
			$t_07jual_detail_list->RestoreFormValues(); // Restore form values
		}
		...
		if ($t_07jual_detail->RowType == EW_ROWTYPE_EDIT) // Edit row
			$t_07jual_detail_list->EditRowCnt++;

		// Set up row id / data-rowindex
		$t_07jual_detail->RowAttrs = array_merge($t_07jual_detail->RowAttrs, array('data-rowindex'=>$t_07jual_detail_list->RowCnt, 'id'=>'r' . $t_07jual_detail_list->RowCnt . '_t_07jual_detail', 'data-rowtype'=>$t_07jual_detail->RowType));
		...
		if ($t_07jual_detail_list->RowAction <> "delete" && $t_07jual_detail_list->RowAction <> "insertdelete" && !($t_07jual_detail_list->RowAction == "insert" && $t_07jual_detail->CurrentAction == "F" && $t_07jual_detail_list->EmptyRow())) {
?>
	<tr<?php echo $t_07jual_detail->RowAttributes() ?>>
<?php
...
<span id="el<?php echo $t_07jual_detail_list->RowCnt ?>_t_07jual_detail_tgl_kirim" class="t_07jual_detail_tgl_kirim">
<span<?php echo $t_07jual_detail->tgl_kirim->ViewAttributes() ?>>
<?php echo $t_07jual_detail->tgl_kirim->ListViewValue() ?></span>
</span>
<?php } ?>
<a id="<?php echo $t_07jual_detail_list->PageObjName . "_row_" . $t_07jual_detail_list->RowCnt ?>"></a></td>
	<?php } ?>
<?php if ($t_07jual_detail->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<input type="hidden" data-table="t_07jual_detail" data-field="x_jual_detail_id" name="x<?php echo $t_07jual_detail_list->RowIndex ?>_jual_detail_id" id="x<?php echo $t_07jual_detail_list->RowIndex ?>_jual_detail_id" value="<?php echo ew_HtmlEncode($t_07jual_detail->jual_detail_id->CurrentValue) ?>">
<input type="hidden" data-table="t_07jual_detail" data-field="x_jual_detail_id" name="o<?php echo $t_07jual_detail_list->RowIndex ?>_jual_detail_id" id="o<?php echo $t_07jual_detail_list->RowIndex ?>_jual_detail_id" value="<?php echo ew_HtmlEncode($t_07jual_detail->jual_detail_id->OldValue) ?>">
<?php } ?>
...
<?php } ?>
<?php if ($t_07jual_detail->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $t_07jual_detail_list->RowCnt ?>_t_07jual_detail_item_id" class="t_07jual_detail_item_id">
...
<?php } ?>
<?php if ($t_07jual_detail->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $t_07jual_detail_list->RowCnt ?>_t_07jual_detail_qty" class="t_07jual_detail_qty">
...
<?php } ?>
<?php if ($t_07jual_detail->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $t_07jual_detail_list->RowCnt ?>_t_07jual_detail_satuan_id" class="t_07jual_detail_satuan_id">
...
<?php } ?>
<?php if ($t_07jual_detail->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $t_07jual_detail_list->RowCnt ?>_t_07jual_detail_harga" class="t_07jual_detail_harga">
...
<?php } ?>
<?php if ($t_07jual_detail->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $t_07jual_detail_list->RowCnt ?>_t_07jual_detail_sub_total" class="t_07jual_detail_sub_total">
...
<?php
	...
	if ($t_07jual_detail->CurrentAction <> "gridadd")
		if (!$t_07jual_detail_list->Recordset->EOF) $t_07jual_detail_list->Recordset->MoveNext();
}
?>
<?php
	if ($t_07jual_detail->CurrentAction == "gridadd" || $t_07jual_detail->CurrentAction == "gridedit") {
		$t_07jual_detail_list->RowIndex = '$rowindex$';
		$t_07jual_detail_list->LoadDefaultValues();

		// Set row properties
		$t_07jual_detail->ResetAttrs();
		$t_07jual_detail->RowAttrs = array_merge($t_07jual_detail->RowAttrs, array('data-rowindex'=>$t_07jual_detail_list->RowIndex, 'id'=>'r0_t_07jual_detail', 'data-rowtype'=>EW_ROWTYPE_ADD));
...
// Render list options (body, left)
$t_07jual_detail_list->ListOptions->Render("body", "left", $t_07jual_detail_list->RowIndex);
?>
	<?php if ($t_07jual_detail->tgl_kirim->Visible) { // tgl_kirim ?>
		<td data-name="tgl_kirim">
...
<?php } ?>
<?php if ($t_07jual_detail->CurrentAction == "add" || $t_07jual_detail->CurrentAction == "copy") { ?>
<input type="hidden" name="<?php echo $t_07jual_detail_list->FormKeyCountName ?>" id="<?php echo $t_07jual_detail_list->FormKeyCountName ?>" value="<?php echo $t_07jual_detail_list->KeyCount ?>">
<?php } ?>
<?php if ($t_07jual_detail->CurrentAction == "gridadd") { ?>
<input type="hidden" name="a_list" id="a_list" value="gridinsert">
<input type="hidden" name="<?php echo $t_07jual_detail_list->FormKeyCountName ?>" id="<?php echo $t_07jual_detail_list->FormKeyCountName ?>" value="<?php echo $t_07jual_detail_list->KeyCount ?>">
<?php echo $t_07jual_detail_list->MultiSelectKey ?>
<?php } ?>
<?php if ($t_07jual_detail->CurrentAction == "edit") { ?>
<input type="hidden" name="<?php echo $t_07jual_detail_list->FormKeyCountName ?>" id="<?php echo $t_07jual_detail_list->FormKeyCountName ?>" value="<?php echo $t_07jual_detail_list->KeyCount ?>">
<?php } ?>
<?php if ($t_07jual_detail->CurrentAction == "gridedit") { ?>
<input type="hidden" name="a_list" id="a_list" value="gridupdate">
<input type="hidden" name="<?php echo $t_07jual_detail_list->FormKeyCountName ?>" id="<?php echo $t_07jual_detail_list->FormKeyCountName ?>" value="<?php echo $t_07jual_detail_list->KeyCount ?>">
<?php echo $t_07jual_detail_list->MultiSelectKey ?>
<?php } ?>
<?php if ($t_07jual_detail->CurrentAction == "") { ?>
...
// Close recordset
if ($t_07jual_detail_list->Recordset)
	$t_07jual_detail_list->Recordset->Close();
?>
<?php if ($t_07jual_detail->Export == "") { ?>
...
<?php } ?>
<?php if ($t_07jual_detail_list->TotalRecs == 0 && $t_07jual_detail->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($t_07jual_detail_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($t_07jual_detail->Export == "") { ?>
<script type="text/javascript">
ft_07jual_detaillist.Init();
</script>
<?php } ?>
<?php
$t_07jual_detail_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($t_07jual_detail->Export == "") { ?>
...
<?php
$t_07jual_detail_list->Page_Terminate();
?>
