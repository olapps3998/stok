...
//
// Table class for t_11dead_stok
//
class ct_11dead_stok extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 't_11dead_stok';
		$this->TableName = 't_11dead_stok';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t_11dead_stok`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// dead_stok_id
		$this->dead_stok_id = new cField('t_11dead_stok', 't_11dead_stok', 'x_dead_stok_id', 'dead_stok_id', '`dead_stok_id`', '`dead_stok_id`', 3, -1, FALSE, '`dead_stok_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->dead_stok_id->Sortable = TRUE; // Allow sort
		$this->dead_stok_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['dead_stok_id'] = &$this->dead_stok_id;

		// tgl
		$this->tgl = new cField('t_11dead_stok', 't_11dead_stok', 'x_tgl', 'tgl', '`tgl`', ew_CastDateFieldForLike('`tgl`', 7, "DB"), 133, 7, FALSE, '`tgl`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// item_id
		$this->item_id = new cField('t_11dead_stok', 't_11dead_stok', 'x_item_id', 'item_id', '`item_id`', '`item_id`', 3, -1, FALSE, '`item_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// qty
		$this->qty = new cField('t_11dead_stok', 't_11dead_stok', 'x_qty', 'qty', '`qty`', '`qty`', 4, -1, FALSE, '`qty`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->qty->Sortable = TRUE; // Allow sort
		$this->qty->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['qty'] = &$this->qty;

		// satuan_id
		$this->satuan_id = new cField('t_11dead_stok', 't_11dead_stok', 'x_satuan_id', 'satuan_id', '`satuan_id`', '`satuan_id`', 3, -1, FALSE, '`satuan_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`t_11dead_stok`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->dead_stok_id->setDbValue($conn->Insert_ID());
			$rs['dead_stok_id'] = $this->dead_stok_id->DbValue;
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('dead_stok_id', $rs))
				ew_AddFilter($where, ew_QuotedName('dead_stok_id', $this->DBID) . '=' . ew_QuotedValue($rs['dead_stok_id'], $this->dead_stok_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`dead_stok_id` = @dead_stok_id@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->dead_stok_id->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@dead_stok_id@", ew_AdjustSql($this->dead_stok_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t_11dead_stokview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t_11dead_stokview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "t_11dead_stokadd.php?" . $this->UrlParm($parm);
		else
			$url = "t_11dead_stokadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("t_11dead_stokedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("t_11dead_stokadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("t_11dead_stokdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "dead_stok_id:" . ew_VarToJson($this->dead_stok_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->dead_stok_id->CurrentValue)) {
			$sUrl .= "dead_stok_id=" . urlencode($this->dead_stok_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["dead_stok_id"]))
				$arKeys[] = ew_StripSlashes($_POST["dead_stok_id"]);
			elseif (isset($_GET["dead_stok_id"]))
				$arKeys[] = ew_StripSlashes($_GET["dead_stok_id"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->dead_stok_id->CurrentValue = $key;
   ...
   // Common render codes
		...
		// dead_stok_id
		$this->dead_stok_id->LinkCustomAttributes = "";
		$this->dead_stok_id->HrefValue = "";
		$this->dead_stok_id->TooltipValue = "";
	...
	function RenderEditRow() {
		...
		// dead_stok_id
		$this->dead_stok_id->EditAttrs["class"] = "form-control";
		$this->dead_stok_id->EditCustomAttributes = "";
		$this->dead_stok_id->EditValue = $this->dead_stok_id->CurrentValue;
		$this->dead_stok_id->ViewCustomAttributes = "";
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->dead_stok_id->Exportable) $Doc->ExportCaption($this->dead_stok_id);
					if ($this->tgl->Exportable) $Doc->ExportCaption($this->tgl);
					if ($this->item_id->Exportable) $Doc->ExportCaption($this->item_id);
					if ($this->qty->Exportable) $Doc->ExportCaption($this->qty);
					if ($this->satuan_id->Exportable) $Doc->ExportCaption($this->satuan_id);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->dead_stok_id->Exportable) $Doc->ExportField($this->dead_stok_id);
						if ($this->tgl->Exportable) $Doc->ExportField($this->tgl);
						if ($this->item_id->Exportable) $Doc->ExportField($this->item_id);
						if ($this->qty->Exportable) $Doc->ExportField($this->qty);
						if ($this->satuan_id->Exportable) $Doc->ExportField($this->satuan_id);
