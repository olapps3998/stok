...
$t_07jual_detail_grid = NULL; // Initialize page object first

class ct_07jual_detail_grid extends ct_07jual_detail {
	...
	// Page object name
	var $PageObjName = 't_07jual_detail_grid';

	// Grid form hidden field names
	var $FormName = 'ft_07jual_detailgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["t_07jual_detail"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->jual_detail_id->CurrentValue = strval($arKeys[0]); // jual_detail_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "t_06jual") {
			$this->jual_id->Visible = FALSE;
			if ($GLOBALS["t_06jual"]->EventCancelled) $this->EventCancelled = TRUE;
		}
