...
while (!$r->EOF) {
	...
	$q1 = "select * from v_12nilai_stok where item_id = ".$item_id."";// order by tgl"; //, jenis desc, detail_id"; echo $q1."</br>";
	$r1 = $conn->Execute($q1);
	while (!$r1->EOF) {
		...
		if ($jenis == "M") { // masuk
			...
			$in_qty = $qty; $in_harga = $harga; $in_sub_total = $qty * $harga; //if ($item_id == "11") {echo $jenis;};
			...
			for ($i = 0; $i <= $a_index; $i++) {
				...
				if ($saldo_qty < $out_qty) { // check lebih besar mana antara saldo dan keluar
					...
					if ($pernah_kurang == "pernah") {
					...
					else { //if ($item_id == 11) {print_r($a_qty); print_r($a_harga); echo $a_index; echo $i; exit;}
						...
						*/
						...
						//if ($item_id == 11) {echo $a_index; print_r($a_qty); echo $jenis; echo $i;}
						for ($i = 1; $i <= $a_index; $i++) {
							$q = "insert into t_09nilai_stok (item_id, item_nama, saldo_qty, saldo_harga, saldo_sub_total) values 
							(".$item_id.", '".$item_nama."', ".$a_qty[$i].", ".$a_harga[$i].", ".$a_qty[$i] * $a_harga[$i].")"; 
							$conn->Execute($q);
