...
//
// Table class for t_08item_saldo
//
class ct_08item_saldo extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 't_08item_saldo';
		$this->TableName = 't_08item_saldo';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t_08item_saldo`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// sld_id
		$this->sld_id = new cField('t_08item_saldo', 't_08item_saldo', 'x_sld_id', 'sld_id', '`sld_id`', '`sld_id`', 3, -1, FALSE, '`sld_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->sld_id->Sortable = TRUE; // Allow sort
		$this->sld_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		...
		// tgl
		$this->tgl = new cField('t_08item_saldo', 't_08item_saldo', 'x_tgl', 'tgl', '`tgl`', ew_CastDateFieldForLike('`tgl`', 7, "DB"), 133, 7, FALSE, '`tgl`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// qty
		$this->qty = new cField('t_08item_saldo', 't_08item_saldo', 'x_qty', 'qty', '`qty`', '`qty`', 4, -1, FALSE, '`qty`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->qty->Sortable = TRUE; // Allow sort
		$this->qty->FldDefaultErrMsg = $Language->Phrase("IncorrectFloat");
		$this->fields['qty'] = &$this->qty;

		// harga
		$this->harga = new cField('t_08item_saldo', 't_08item_saldo', 'x_harga', 'harga', '`harga`', '`harga`', 4, -1, FALSE, '`harga`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`t_08item_saldo`";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->sld_id->setDbValue($conn->Insert_ID());
			$rs['sld_id'] = $this->sld_id->DbValue;
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('sld_id', $rs))
				ew_AddFilter($where, ew_QuotedName('sld_id', $this->DBID) . '=' . ew_QuotedValue($rs['sld_id'], $this->sld_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`sld_id` = @sld_id@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->sld_id->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@sld_id@", ew_AdjustSql($this->sld_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t_08item_saldoview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t_08item_saldoview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "t_08item_saldoadd.php?" . $this->UrlParm($parm);
		else
			$url = "t_08item_saldoadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("t_08item_saldoedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("t_08item_saldoadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("t_08item_saldodelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "sld_id:" . ew_VarToJson($this->sld_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->sld_id->CurrentValue)) {
			$sUrl .= "sld_id=" . urlencode($this->sld_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["sld_id"]))
				$arKeys[] = ew_StripSlashes($_POST["sld_id"]);
			elseif (isset($_GET["sld_id"]))
				$arKeys[] = ew_StripSlashes($_GET["sld_id"]);
			else
   ...
   // Common render codes
		...
		// sld_id
		$this->sld_id->LinkCustomAttributes = "";
		$this->sld_id->HrefValue = "";
		$this->sld_id->TooltipValue = "";
	...
	function RenderEditRow() {
		...
		// sld_id
		$this->sld_id->EditAttrs["class"] = "form-control";
		$this->sld_id->EditCustomAttributes = "";
		$this->sld_id->EditValue = $this->sld_id->CurrentValue;
		$this->sld_id->ViewCustomAttributes = "";
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->sld_id->Exportable) $Doc->ExportCaption($this->sld_id);
					if ($this->item_id->Exportable) $Doc->ExportCaption($this->item_id);
					if ($this->tgl->Exportable) $Doc->ExportCaption($this->tgl);
					if ($this->qty->Exportable) $Doc->ExportCaption($this->qty);
					if ($this->harga->Exportable) $Doc->ExportCaption($this->harga);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->sld_id->Exportable) $Doc->ExportField($this->sld_id);
						if ($this->item_id->Exportable) $Doc->ExportField($this->item_id);
						if ($this->tgl->Exportable) $Doc->ExportField($this->tgl);
						if ($this->qty->Exportable) $Doc->ExportField($this->qty);
						if ($this->harga->Exportable) $Doc->ExportField($this->harga);
