...
function ew_InitPasswordOptions() {
	...
	if ($.fn.pStrength && typeof EW_MIN_PASSWORD_STRENGTH != "undefined") {
		$(".ewPasswordStrength").each(function() {
			var $this = $(this);
	...
	if ($.fn.pGenerator) {
		$(".ewPasswordGenerator").each(function() {
			var $this = $(this);
...
jQuery(function($) {
	...
	ew_SetSessionTimer();
	$(".ewPopover").popover();
	$(".ewTooltip").tooltip({ "placement": "bottom" });
	ew_InitIcons();
	...
	if (typeof EW_USE_JAVASCRIPT_MESSAGE != "undefined" && EW_USE_JAVASCRIPT_MESSAGE) {
		...
		$("#ewModalDialog").on("load.ew", ew_ShowMessage); // Show message for modal dialog
	}
});
...
//			}
	...
	if (!$.isBoolean(async)) { // Non-Ajax
		...
		if ((ew_IsAutoSuggest(obj) || ew_IsModalLookup(obj)) && this.Form) // Auto-Suggest/Modal-Lookup (init form or auto-fill)
...
function ew_ShowTemplates(classname) {
	...
	$("script" + ((classname) ? "." + classname : "") + "[type='text/html']").each(function() {
		...
		$scr.closest(".ewTable, .ewViewTable").show();
...
function ew_IsHidden(el) {
	var $ = jQuery;
	return $(el).css("display") == "none" && !$(el).closest(".dropdown-menu")[0] && !ew_IsModalLookup(el) && !ew_IsHiddenTextArea(el) && !ew_IsAutoSuggest(el) || ew_GetAncestorBy(el, function(node) {
			var $node = $(node);
		...
		var o = {
			...
			remote: {
				...
				prepare: function(query, settings) {
					...
					settings.type = "POST";
					settings.url += "?q=" + encodeURIComponent(query) + "&rnd=" + ew_Random();
					return settings;
				},
	...
	$body.ewjtable({
		...
		recordsLoaded: function(e, data) {
			var selectedRows = $(e.target).find(".ewjtable-data-row").filter(function() {
				return ew_InArray($(this).data("record-key"), arLinkValue) > -1;
			});
		...
		selectionChanged: function(e, data) {
			...
			if ($rows) {
				...
				$rows.each(function() {
					...
					var index = ew_InArray(key, arLinkValue);
...
function ew_Upload(input) {
	...
	$input.fileupload({
		...
		filesContainer: $ft,
		formData: {id: id, table: tbl, replace: (multiple ? "0" : "1"), exts: $exts.val(), maxsize: $maxsize.val(), maxfilecount: $maxfilecount.val()},
...
function ew_ParseDate(data, format) {
	...
	if ($.isNumber(format) && format >=0 && format <= 17) {
		...
		if (format == 0) // Default
			format = EW_DATE_FORMAT_ID;
		switch (format) {
			case 1: f = def + " HH" + timesep + "mm" + timesep + "ss"; break; // EW_DATE_FORMAT + " %H:%M:%S"
			case 2: f = def; break; // EW_DATE_FORMAT
