...
<?php include_once "r_mutasismryinfo.php" ?>
<?php
...
$r_mutasi_summary = NULL; // Initialize page object first

class crr_mutasi_summary extends crr_mutasi {
	...
	// Page object name
	var $PageObjName = 'r_mutasi_summary';
	...
	function __construct() {
		...
		// Table object (r_mutasi)
		if (!isset($GLOBALS["r_mutasi"])) {
			$GLOBALS["r_mutasi"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["r_mutasi"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'r_mutasi', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fr_mutasisummary";
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 'r_mutasi');
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_r_mutasi\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_r_mutasi',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fr_mutasisummary\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fr_mutasisummary\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fr_mutasisummary\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function RenderRow() {
		...
		if (!$this->GrandSummarySetup) { // Get Grand total
			...
			if ($rsagg) {
				...
				$this->GrandSmry[2] = $rsagg->fields("sum_masuk");
	...
	function GetExtendedFilter() {
		...
		// Reset search command
			...
			if ($this->GetDropDownValue($this->item_nama)) {
				$bSetupFilter = TRUE;
			} elseif ($this->item_nama->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_r_mutasi_item_nama'])) {
				$bSetupFilter = TRUE;
			}
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_r_mutasi_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_r_mutasi_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_r_mutasi_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_r_mutasi_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_r_mutasi_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_r_mutasi_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_r_mutasi_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_r_mutasi_' . $parm] = $sv;
		$_SESSION['so_r_mutasi_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_r_mutasi_' . $parm] = $sv1;
		$_SESSION['so_r_mutasi_' . $parm] = $so1;
		$_SESSION['sc_r_mutasi_' . $parm] = $sc;
		$_SESSION['sv2_r_mutasi_' . $parm] = $sv2;
		$_SESSION['so2_r_mutasi_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_r_mutasi_$parm"] = "";
		$_SESSION["rf_r_mutasi_$parm"] = "";
		$_SESSION["rt_r_mutasi_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->FieldByParm($parm);
		$fld->SelectionList = @$_SESSION["sel_r_mutasi_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_r_mutasi_$parm"];
		$fld->RangeTo = @$_SESSION["rt_r_mutasi_$parm"];
	}
...
// Create page object
if (!isset($r_mutasi_summary)) $r_mutasi_summary = new crr_mutasi_summary();
if (isset($Page)) $OldPage = $Page;
$Page = &$r_mutasi_summary;
...
// Create page object
var r_mutasi_summary = new ewr_Page("r_mutasi_summary");

// Page properties
r_mutasi_summary.PageID = "summary"; // Page ID
var EWR_PAGE_ID = r_mutasi_summary.PageID;

// Extend page with Chart_Rendering function
r_mutasi_summary.Chart_Rendering = 
...
r_mutasi_summary.Chart_Rendered = 
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
// Form object
var CurrentForm = fr_mutasisummary = new ewr_Form("fr_mutasisummary");

// Validate method
fr_mutasisummary.Validate = function() {
...
fr_mutasisummary.Form_CustomValidate = 
...
fr_mutasisummary.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fr_mutasisummary.ValidateRequired = false; // No JavaScript validation
<?php } ?>

// Use Ajax
fr_mutasisummary.Lists["sv_item_nama"] = {"LinkField":"sv_item_nama","Ajax":true,"DisplayFields":["sv_item_nama","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
</script>
<?php } ?>
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<script type="text/javascript">
...
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<!-- Search form (begin) -->
<form name="fr_mutasisummary" id="fr_mutasisummary" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fr_mutasisummary_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<select data-table="r_mutasi" data-field="x_item_nama" data-value-separator="<?php echo ewr_HtmlEncode(is_array($Page->item_nama->DisplayValueSeparator) ? json_encode($Page->item_nama->DisplayValueSeparator) : $Page->item_nama->DisplayValueSeparator) ?>" id="sv_item_nama" name="sv_item_nama"<?php echo $Page->item_nama->EditAttributes() ?>>
...
<input type="text" data-table="r_mutasi" data-field="x_tgl" id="sv_tgl" name="sv_tgl" placeholder="<?php echo $Page->tgl->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->tgl->SearchValue) ?>" data-calendar="true" data-formatid="7"<?php echo $Page->tgl->EditAttributes() ?>>
</span>
	<span class="ewSearchCond btw1_tgl"><?php echo $ReportLanguage->Phrase("AND") ?></span>
	<span class="ewSearchField btw1_tgl">
<?php ewr_PrependClass($Page->tgl->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" data-table="r_mutasi" data-field="x_tgl" id="sv2_tgl" name="sv2_tgl" placeholder="<?php echo $Page->tgl->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->tgl->SearchValue2) ?>" data-calendar="true" data-formatid="7"<?php echo $Page->tgl->EditAttributes() ?>>
</span>
</div>
</div>
...
<script type="text/javascript">
fr_mutasisummary.Init();
fr_mutasisummary.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<?php include "r_mutasismrypager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php } ?>
...
<?php } ?>
<span data-class="tpb<?php echo $Page->GrpCount-1 ?>_r_mutasi"><?php echo $Page->PageBreakContent ?></span>
<?php } ?>
<?php if ($Page->Export <> "pdf") { ?>
...
<?php if ($Page->item_id->Visible) { ?>
	...
	<?php } else { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="item_id"><div class="r_mutasi_item_id"><span class="ewTableHeaderCaption"><?php echo $Page->item_id->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="item_id">
<?php if ($Page->SortUrl($Page->item_id) == "") { ?>
		<div class="ewTableHeaderBtn r_mutasi_item_id">
			<span class="ewTableHeaderCaption"><?php echo $Page->item_id->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_mutasi_item_id" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->item_id) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->item_id->FldCaption() ?></span>
...
<?php if ($Page->item_nama->Visible) { ?>
	...
	<?php } else { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="item_nama"><div class="r_mutasi_item_nama"><span class="ewTableHeaderCaption"><?php echo $Page->item_nama->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="item_nama">
<?php if ($Page->SortUrl($Page->item_nama) == "") { ?>
		<div class="ewTableHeaderBtn r_mutasi_item_nama">
			<span class="ewTableHeaderCaption"><?php echo $Page->item_nama->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_mutasi_item_nama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->item_nama) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->item_nama->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="tgl"><div class="r_mutasi_tgl"><span class="ewTableHeaderCaption"><?php echo $Page->tgl->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="tgl">
<?php if ($Page->SortUrl($Page->tgl) == "") { ?>
		<div class="ewTableHeaderBtn r_mutasi_tgl">
			<span class="ewTableHeaderCaption"><?php echo $Page->tgl->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_mutasi_tgl" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->tgl) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->tgl->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="masuk"><div class="r_mutasi_masuk" style="text-align: right;"><span class="ewTableHeaderCaption"><?php echo $Page->masuk->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="masuk">
<?php if ($Page->SortUrl($Page->masuk) == "") { ?>
		<div class="ewTableHeaderBtn r_mutasi_masuk" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->masuk->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_mutasi_masuk" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->masuk) ?>',2);" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->masuk->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="keluar"><div class="r_mutasi_keluar" style="text-align: right;"><span class="ewTableHeaderCaption"><?php echo $Page->keluar->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="keluar">
<?php if ($Page->SortUrl($Page->keluar) == "") { ?>
		<div class="ewTableHeaderBtn r_mutasi_keluar" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->keluar->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_mutasi_keluar" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->keluar) ?>',2);" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->keluar->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="saldo"><div class="r_mutasi_saldo" style="text-align: right;"><span class="ewTableHeaderCaption"><?php echo $Page->saldo->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="saldo">
<?php if ($Page->SortUrl($Page->saldo) == "") { ?>
		<div class="ewTableHeaderBtn r_mutasi_saldo" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->saldo->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_mutasi_saldo" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->saldo) ?>',2);" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->saldo->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
		<span class="ewSummaryCaption r_mutasi_item_id"><span class="ewTableHeaderCaption"><?php echo $Page->item_id->FldCaption() ?></span></span>
<?php } else { ?>
	<?php if ($Page->SortUrl($Page->item_id) == "") { ?>
		<span class="ewSummaryCaption r_mutasi_item_id">
			<span class="ewTableHeaderCaption"><?php echo $Page->item_id->FldCaption() ?></span>
		</span>
	<?php } else { ?>
		<span class="ewTableHeaderBtn ewPointer ewSummaryCaption r_mutasi_item_id" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->item_id) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->item_id->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->item_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->item_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</span>
	<?php } ?>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_r_mutasi_item_id"<?php echo $Page->item_id->ViewAttributes() ?>><?php echo $Page->item_id->GroupViewValue ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
		<span class="ewSummaryCaption r_mutasi_item_nama"><span class="ewTableHeaderCaption"><?php echo $Page->item_nama->FldCaption() ?></span></span>
<?php } else { ?>
	<?php if ($Page->SortUrl($Page->item_nama) == "") { ?>
		<span class="ewSummaryCaption r_mutasi_item_nama">
			<span class="ewTableHeaderCaption"><?php echo $Page->item_nama->FldCaption() ?></span>
		</span>
	<?php } else { ?>
		<span class="ewTableHeaderBtn ewPointer ewSummaryCaption r_mutasi_item_nama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->item_nama) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->item_nama->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->item_nama->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->item_nama->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</span>
	<?php } ?>
<?php } ?>
		<?php echo $ReportLanguage->Phrase("SummaryColon") ?>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_r_mutasi_item_nama"<?php echo $Page->item_nama->ViewAttributes() ?>><?php echo $Page->item_nama->GroupViewValue ?></span>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_r_mutasi_item_id"<?php echo $Page->item_id->ViewAttributes() ?>><?php echo $Page->item_id->GroupViewValue ?></span></td>
	<?php } ?>
<?php } ?>
<?php if ($Page->item_nama->Visible) { ?>
	<?php if ($Page->item_nama->ShowGroupHeaderAsRow) { ?>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_r_mutasi_item_nama"<?php echo $Page->item_nama->ViewAttributes() ?>><?php echo $Page->item_nama->GroupViewValue ?></span></td>
	<?php } ?>
<?php } ?>
<?php if ($Page->tgl->Visible) { ?>
		<td data-field="tgl"<?php echo $Page->tgl->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->RecCount ?>_r_mutasi_tgl"<?php echo $Page->tgl->ViewAttributes() ?>><?php echo $Page->tgl->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->masuk->Visible) { ?>
		<td data-field="masuk"<?php echo $Page->masuk->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->RecCount ?>_r_mutasi_masuk"<?php echo $Page->masuk->ViewAttributes() ?>><?php echo $Page->masuk->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->keluar->Visible) { ?>
		<td data-field="keluar"<?php echo $Page->keluar->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->RecCount ?>_r_mutasi_keluar"<?php echo $Page->keluar->ViewAttributes() ?>><?php echo $Page->keluar->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->saldo->Visible) { ?>
...
<?php
			...
			$Page->masuk->Count = $Page->Cnt[1][2];
			$Page->masuk->SumValue = $Page->Smry[1][2]; // Load SUM
			$Page->keluar->Count = $Page->Cnt[1][3];
			$Page->keluar->SumValue = $Page->Smry[1][3]; // Load SUM
			$Page->ResetAttrs();
			$Page->RowType = EWR_ROWTYPE_TOTAL;
			$Page->RowTotalType = EWR_ROWTOTAL_GROUP;
