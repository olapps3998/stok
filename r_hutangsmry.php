...
<?php include_once "r_hutangsmryinfo.php" ?>
<?php
...
$r_hutang_summary = NULL; // Initialize page object first

class crr_hutang_summary extends crr_hutang {
	...
	// Page object name
	var $PageObjName = 'r_hutang_summary';
	...
	function __construct() {
		...
		// Table object (r_hutang)
		if (!isset($GLOBALS["r_hutang"])) {
			$GLOBALS["r_hutang"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["r_hutang"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'r_hutang', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fr_hutangsummary";
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 'r_hutang');
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_r_hutang\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_r_hutang',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fr_hutangsummary\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fr_hutangsummary\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fr_hutangsummary\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function RenderRow() {
		...
		if (!$this->GrandSummarySetup) { // Get Grand total
			...
			if ($rsagg) {
				...
				$this->GrandSmry[2] = $rsagg->fields("sum_tot_hutang");
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL && !($this->RowTotalType == EWR_ROWTOTAL_GROUP && $this->RowTotalSubType == EWR_ROWTOTAL_HEADER)) { // Summary row
			...
			// tot_hutang
			$this->tot_hutang->SumViewValue = $this->tot_hutang->SumValue;
			$this->tot_hutang->SumViewValue = ewr_FormatNumber($this->tot_hutang->SumViewValue, 0, -2, -2, -2);
			$this->tot_hutang->CellAttrs["style"] = "text-align:right;";
			$this->tot_hutang->CellAttrs["class"] = ($this->RowTotalType == EWR_ROWTOTAL_PAGE || $this->RowTotalType == EWR_ROWTOTAL_GRAND) ? "ewRptGrpAggregate" : "ewRptGrpSummary" . $this->RowGroupLevel;

			// tot_dp
			$this->tot_dp->SumViewValue = $this->tot_dp->SumValue;
			$this->tot_dp->SumViewValue = ewr_FormatNumber($this->tot_dp->SumViewValue, 0, -2, -2, -2);
			$this->tot_dp->CellAttrs["style"] = "text-align:right;";
			$this->tot_dp->CellAttrs["class"] = ($this->RowTotalType == EWR_ROWTOTAL_PAGE || $this->RowTotalType == EWR_ROWTOTAL_GRAND) ? "ewRptGrpAggregate" : "ewRptGrpSummary" . $this->RowGroupLevel;

			// tot_lunas
			$this->tot_lunas->SumViewValue = $this->tot_lunas->SumValue;
			$this->tot_lunas->SumViewValue = ewr_FormatNumber($this->tot_lunas->SumViewValue, 0, -2, -2, -2);
			...
			// tot_hutang
			$this->tot_hutang->HrefValue = "";

			// tot_dp
			$this->tot_dp->HrefValue = "";
			...
			// tot_hutang
			$this->tot_hutang->ViewValue = $this->tot_hutang->CurrentValue;
			$this->tot_hutang->ViewValue = ewr_FormatNumber($this->tot_hutang->ViewValue, 0, -2, -2, -2);
			$this->tot_hutang->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
			$this->tot_hutang->CellAttrs["style"] = "text-align:right;";

			// tot_dp
			$this->tot_dp->ViewValue = $this->tot_dp->CurrentValue;
			$this->tot_dp->ViewValue = ewr_FormatNumber($this->tot_dp->ViewValue, 0, -2, -2, -2);
			$this->tot_dp->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
			$this->tot_dp->CellAttrs["style"] = "text-align:right;";

			// tot_lunas
			$this->tot_lunas->ViewValue = $this->tot_lunas->CurrentValue;
			$this->tot_lunas->ViewValue = ewr_FormatNumber($this->tot_lunas->ViewValue, 0, -2, -2, -2);
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// tot_hutang
			$CurrentValue = $this->tot_hutang->SumValue;
			$ViewValue = &$this->tot_hutang->SumViewValue;
			$ViewAttrs = &$this->tot_hutang->ViewAttrs;
			$CellAttrs = &$this->tot_hutang->CellAttrs;
			$HrefValue = &$this->tot_hutang->HrefValue;
			$LinkAttrs = &$this->tot_hutang->LinkAttrs;
			$this->Cell_Rendered($this->tot_hutang, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);

			// tot_dp
			$CurrentValue = $this->tot_dp->SumValue;
			$ViewValue = &$this->tot_dp->SumViewValue;
			$ViewAttrs = &$this->tot_dp->ViewAttrs;
			$CellAttrs = &$this->tot_dp->CellAttrs;
			$HrefValue = &$this->tot_dp->HrefValue;
			$LinkAttrs = &$this->tot_dp->LinkAttrs;
			$this->Cell_Rendered($this->tot_dp, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);

			// tot_lunas
			$CurrentValue = $this->tot_lunas->SumValue;
			$ViewValue = &$this->tot_lunas->SumViewValue;
			$ViewAttrs = &$this->tot_lunas->ViewAttrs;
			$CellAttrs = &$this->tot_lunas->CellAttrs;
			...
			// tot_hutang
			$CurrentValue = $this->tot_hutang->CurrentValue;
			$ViewValue = &$this->tot_hutang->ViewValue;
			$ViewAttrs = &$this->tot_hutang->ViewAttrs;
			$CellAttrs = &$this->tot_hutang->CellAttrs;
			$HrefValue = &$this->tot_hutang->HrefValue;
			$LinkAttrs = &$this->tot_hutang->LinkAttrs;
			...
			// tot_dp
			$CurrentValue = $this->tot_dp->CurrentValue;
			$ViewValue = &$this->tot_dp->ViewValue;
			$ViewAttrs = &$this->tot_dp->ViewAttrs;
			$CellAttrs = &$this->tot_dp->CellAttrs;
			$HrefValue = &$this->tot_dp->HrefValue;
			$LinkAttrs = &$this->tot_dp->LinkAttrs;
			...
			// tot_lunas
			$CurrentValue = $this->tot_lunas->CurrentValue;
			$ViewValue = &$this->tot_lunas->ViewValue;
			$ViewAttrs = &$this->tot_lunas->ViewAttrs;
			$CellAttrs = &$this->tot_lunas->CellAttrs;
			$HrefValue = &$this->tot_lunas->HrefValue;
			$LinkAttrs = &$this->tot_lunas->LinkAttrs;
	...
	function GetExtendedFilter() {
		...
		// Reset search command
			...
			if ($this->GetDropDownValue($this->vendor_nama)) {
				$bSetupFilter = TRUE;
			} elseif ($this->vendor_nama->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_r_hutang_vendor_nama'])) {
				$bSetupFilter = TRUE;
			}
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_r_hutang_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_r_hutang_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_r_hutang_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_r_hutang_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_r_hutang_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_r_hutang_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_r_hutang_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_r_hutang_' . $parm] = $sv;
		$_SESSION['so_r_hutang_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_r_hutang_' . $parm] = $sv1;
		$_SESSION['so_r_hutang_' . $parm] = $so1;
		$_SESSION['sc_r_hutang_' . $parm] = $sc;
		$_SESSION['sv2_r_hutang_' . $parm] = $sv2;
		$_SESSION['so2_r_hutang_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_r_hutang_$parm"] = "";
		$_SESSION["rf_r_hutang_$parm"] = "";
		$_SESSION["rt_r_hutang_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->FieldByParm($parm);
		$fld->SelectionList = @$_SESSION["sel_r_hutang_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_r_hutang_$parm"];
		$fld->RangeTo = @$_SESSION["rt_r_hutang_$parm"];
	}
	...
	function GetSort($options = array()) {
		...
		// Set up default sort
		if ($this->getOrderBy() == "") {
			$this->setOrderBy("`vendor_nama` ASC");
			$this->vendor_nama->setSort("ASC");
		}
...
// Create page object
if (!isset($r_hutang_summary)) $r_hutang_summary = new crr_hutang_summary();
if (isset($Page)) $OldPage = $Page;
$Page = &$r_hutang_summary;
...
// Create page object
var r_hutang_summary = new ewr_Page("r_hutang_summary");

// Page properties
r_hutang_summary.PageID = "summary"; // Page ID
var EWR_PAGE_ID = r_hutang_summary.PageID;

// Extend page with Chart_Rendering function
r_hutang_summary.Chart_Rendering = 
...
r_hutang_summary.Chart_Rendered = 
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
// Form object
var CurrentForm = fr_hutangsummary = new ewr_Form("fr_hutangsummary");

// Validate method
fr_hutangsummary.Validate = function() {
...
fr_hutangsummary.Form_CustomValidate = 
...
fr_hutangsummary.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fr_hutangsummary.ValidateRequired = false; // No JavaScript validation
<?php } ?>

// Use Ajax
fr_hutangsummary.Lists["sv_vendor_nama"] = {"LinkField":"sv_vendor_nama","Ajax":true,"DisplayFields":["sv_vendor_nama","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
</script>
<?php } ?>
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<script type="text/javascript">
...
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<!-- Search form (begin) -->
<form name="fr_hutangsummary" id="fr_hutangsummary" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fr_hutangsummary_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<script type="text/javascript">
fr_hutangsummary.Init();
fr_hutangsummary.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<?php include "r_hutangsmrypager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<!-- Report grid (begin) -->
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="vendor_nama"><div class="r_hutang_vendor_nama"><span class="ewTableHeaderCaption"><?php echo $Page->vendor_nama->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="vendor_nama">
<?php if ($Page->SortUrl($Page->vendor_nama) == "") { ?>
		<div class="ewTableHeaderBtn r_hutang_vendor_nama">
			<span class="ewTableHeaderCaption"><?php echo $Page->vendor_nama->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_hutang_vendor_nama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->vendor_nama) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->vendor_nama->FldCaption() ?></span>
...
<?php } ?>
<?php } ?>
<?php if ($Page->tot_hutang->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="tot_hutang"><div class="r_hutang_tot_hutang" style="text-align: right;"><span class="ewTableHeaderCaption"><?php echo $Page->tot_hutang->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="tot_hutang">
<?php if ($Page->SortUrl($Page->tot_hutang) == "") { ?>
		<div class="ewTableHeaderBtn r_hutang_tot_hutang" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->tot_hutang->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_hutang_tot_hutang" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->tot_hutang) ?>',2);" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->tot_hutang->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->tot_hutang->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->tot_hutang->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
<?php } ?>
<?php if ($Page->tot_dp->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="tot_dp"><div class="r_hutang_tot_dp" style="text-align: right;"><span class="ewTableHeaderCaption"><?php echo $Page->tot_dp->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="tot_dp">
<?php if ($Page->SortUrl($Page->tot_dp) == "") { ?>
		<div class="ewTableHeaderBtn r_hutang_tot_dp" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->tot_dp->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_hutang_tot_dp" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->tot_dp) ?>',2);" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->tot_dp->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->tot_dp->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->tot_dp->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="tot_lunas"><div class="r_hutang_tot_lunas" style="text-align: right;"><span class="ewTableHeaderCaption"><?php echo $Page->tot_lunas->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="tot_lunas">
<?php if ($Page->SortUrl($Page->tot_lunas) == "") { ?>
		<div class="ewTableHeaderBtn r_hutang_tot_lunas" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->tot_lunas->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r_hutang_tot_lunas" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->tot_lunas) ?>',2);" style="text-align: right;">
			<span class="ewTableHeaderCaption"><?php echo $Page->tot_lunas->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->tot_lunas->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->tot_lunas->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_r_hutang_vendor_nama"<?php echo $Page->vendor_nama->ViewAttributes() ?>><?php echo $Page->vendor_nama->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->tot_hutang->Visible) { ?>
		<td data-field="tot_hutang"<?php echo $Page->tot_hutang->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_r_hutang_tot_hutang"<?php echo $Page->tot_hutang->ViewAttributes() ?>><?php echo $Page->tot_hutang->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->tot_dp->Visible) { ?>
		<td data-field="tot_dp"<?php echo $Page->tot_dp->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_r_hutang_tot_dp"<?php echo $Page->tot_dp->ViewAttributes() ?>><?php echo $Page->tot_dp->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->tot_lunas->Visible) { ?>
...
<tfoot>
<?php
	$Page->tot_hutang->Count = $Page->GrandCnt[2];
	$Page->tot_hutang->SumValue = $Page->GrandSmry[2]; // Load SUM
	$Page->tot_dp->Count = $Page->GrandCnt[3];
...
<?php } ?>
<?php if ($Page->tot_hutang->Visible) { ?>
		<td data-field="tot_hutang"<?php echo $Page->tot_hutang->CellAttributes() ?>><span class="ewAggregate"><?php echo $ReportLanguage->Phrase("RptSum") ?></span><?php echo $ReportLanguage->Phrase("AggregateColon") ?>
<span data-class="tpts_r_hutang_tot_hutang"<?php echo $Page->tot_hutang->ViewAttributes() ?>><?php echo $Page->tot_hutang->SumViewValue ?></span></td>
<?php } ?>
<?php if ($Page->tot_dp->Visible) { ?>
		<td data-field="tot_dp"<?php echo $Page->tot_dp->CellAttributes() ?>><span class="ewAggregate"><?php echo $ReportLanguage->Phrase("RptSum") ?></span><?php echo $ReportLanguage->Phrase("AggregateColon") ?>
<span data-class="tpts_r_hutang_tot_dp"<?php echo $Page->tot_dp->ViewAttributes() ?>><?php echo $Page->tot_dp->SumViewValue ?></span></td>
<?php } ?>
<?php if ($Page->tot_lunas->Visible) { ?>
